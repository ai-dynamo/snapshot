...
{
  if (result < 0)
    throw std::system_error(errno, std::generic_category(), "allocation socket");
  if (result == 0)
    throw std::runtime_error("allocation socket closed during frame");
}
...
size_t ReceiveBytes(int socket, void* output, size_t size, std::vector<FileDescriptor>& descriptors)
{
  ...
  if (invalid || descriptors.size() > kFrameDescriptorLimit)
    throw std::runtime_error("invalid allocation descriptor frame");
...
bool ReceiveFrame(int socket, google::protobuf::MessageLite& message, std::vector<FileDescriptor>& descriptors)
{
  ...
  if (size > kFrameSizeLimit)
    throw std::runtime_error("allocation frame exceeds limit");
  ...
  if (!message.ParseFromString(body))
    throw std::runtime_error("invalid allocation protobuf");
  return true;
}
...
void SetAllocationTimeout(int socket)
{
  ...
  if (setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) ||
    ...
    throw std::system_error(errno, std::generic_category(), "allocation timeout");
}
}  // namespace snapshot::pagebroker
