...
bool PosixTransfer(bool write, void *buffer, int file_fd,
  ...
  while (completed < length) {
    if (cancellation != nullptr && cancellation->IsCancelled()) {
      *error = cancellation->DeadlineExceeded()
                   ? "POSIX transfer exceeded the configured operation deadline"
                   : "POSIX transfer canceled after another extent failed";
      return false;
    }
    const off_t offset = static_cast<off_t>(file_offset + completed);
    ssize_t result;
    do {
      result = write ? pwrite(file_fd, bytes + completed, length - completed, offset)
                     : pread(file_fd, bytes + completed, length - completed, offset);
    } while (result < 0 && errno == EINTR);
    if (result < 0) {
      *error = std::string(write ? "pwrite" : "pread") +
               " failed: " + std::strerror(errno);
      return false;
    }
    if (result == 0) {
      *error = write ? "pwrite made no progress"
...
bool TransferExtent(CUdeviceptr device_ptr, size_t extent_size, CUstream stream,
  ...
  *metrics = {};
  ...
  storage::ContentDigest digest;
  {
    ...
    success = TransferPipeline(chunks, files, &slots, device_ptr, stream,
                               operation, &digest, metrics, cancellation,
                               error);
    if (success) {
