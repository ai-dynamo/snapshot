...
main(int argc, char** argv)
{
  ...
  if ((argc != 6 && argc != 8) || std::string_view(argv[4]) != "--max-concurrent-requests" ||
      (argc == 8 && std::string_view(argv[6]) != "--gpu-engine") ||
      !ParseMaxConcurrentRequests(argv[5], max_concurrent_requests)) {
    std::cerr << "usage: pagebroker socket_path staging_directory storage_root --max-concurrent-requests count [--gpu-engine absolute_path]\n";
