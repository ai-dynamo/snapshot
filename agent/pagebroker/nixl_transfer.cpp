...
// Uses the POSIX async request model from the earlier CUDA transfer engine
// (a79ec05), without native CustomStorage or a second worker wire protocol.
...
bool NixlTransfer::Wait(size_t slot, TransferCancellation* cancellation,
                        double* seconds, std::string* error)
{
  ...
  while (request.status == NIXL_IN_PROG) {
    cancelled |= cancellation && cancellation->IsCancelled();
  ...
  *seconds += std::chrono::duration<double>(Clock::now() - request.started).count();
  ...
  if (request.status != NIXL_SUCCESS || cancelled) {
    *error = cancelled ? "NIXL transfer cancelled" : "NIXL transfer failed: " + std::to_string(request.status);
    return false;
  }
  return true;
}
...
void NixlTransfer::Close()
{
  ...
  for (size_t slot = 0; slot < impl_->requests.size(); ++slot)
    if (!Wait(slot, nullptr, &seconds, &error))
