...
void Run(int pid, const std::filesystem::path& directory)
{
  ...
  struct stat info{};
  if (!directory.is_absolute() || lstat(directory.c_str(), &info) ||
  ...
  std::fflush(stdout);
  {
    ...
    while (std::getline(std::cin, command)) {
      if (command != "save" && command != "load")
        throw std::runtime_error("expected save or load");
      if (poll(&target, 1, 0) != 0)
        throw std::runtime_error("target exited");
      const bool save = command == "save";
      ...
      const double transfer_time = std::chrono::duration<double>(Clock::now() - transfer_start).count();
      const auto complete_start = Clock::now();
      Check(complete(view->handle), "native COMPLETE");
      const double complete_time = std::chrono::duration<double>(Clock::now() - complete_start).count();
      if (save) {
      ...
      std::printf("{\"event\":\"%s\",\"bytes\":%zu,\"native_prepare_seconds\":%.6f,"
                  "\"transfer_seconds\":%.6f,\"transfer_setup_seconds\":%.6f,"
                  "\"storage_request_service_seconds\":%.6f,"
                  "\"complete_seconds\":%.6f,\"total_seconds\":%.6f}\n",
                  command.c_str(), transferred, prepare, transfer_time, setup_time, storage_time, complete_time,
...
int main(int argc, char** argv)
{
  try {
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != text.data() + text.size() || pid <= 0 || pid == getpid())
