...
class NixlRequestGuard {
public:
  NixlRequestGuard(nixlAgent *agent, nixlXferReqH *request)
      : agent_(agent), request_(request) {}
  NixlRequestGuard(const NixlRequestGuard &) = delete;
  NixlRequestGuard &operator=(const NixlRequestGuard &) = delete;
  ...
  ~NixlRequestGuard() {
    ...
    if (in_progress_) {
      nixl_status_t status = agent_->getXferStatus(request_);
      while (status == NIXL_IN_PROG) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        status = agent_->getXferStatus(request_);
      }
    }
    (void)agent_->releaseXferReq(request_);
  }

  void MarkInProgress() { in_progress_ = true; }

  nixl_status_t ReleaseTerminal() {
    // Do not retry an ambiguous release result. The agent remains alive through
    // registration cleanup and owns any residual request state.
    nixlXferReqH *request = std::exchange(request_, nullptr);
    return agent_->releaseXferReq(request);
  }

private:
  nixlAgent *agent_;
  nixlXferReqH *request_;
...
bool OpenStorageFiles(const StorageLayout &storage, TransferOperation operation,
  ...
  for (size_t index = 0; index < storage.files.size(); ++index) {
    ...
    if (file.descriptor_fd >= 0) {
      if (operation != TransferOperation::kRestore || file.path.empty() ||
          file.path.is_absolute() || !file.path.parent_path().empty()) {
        *error = "storage file " + std::to_string(index) +
                 " has an invalid pinned descriptor path";
        return false;
      }
      const int duplicate = fcntl(file.descriptor_fd, F_DUPFD_CLOEXEC, 0);
      if (duplicate < 0) {
        *error = "duplicate pinned storage file " + std::to_string(index) +
                 " failed: " + std::strerror(errno);
        return false;
      }
      FileDescriptor descriptor(duplicate);
      ...
      if (fstat(descriptor.get(), &file_stat) != 0 ||
          ...
          static_cast<uint64_t>(file_stat.st_dev) != file.device ||
          static_cast<uint64_t>(file_stat.st_ino) != file.inode) {
        *error = "pinned restore storage file " + std::to_string(index) +
    ...
    if (file.directory_fd >= 0) {
      if (operation != TransferOperation::kRestore || file.path.empty() ||
          file.path.is_absolute() || !file.path.parent_path().empty() ||
          file.path.filename() == "." || file.path.filename() == "..") {
        *error = "storage file " + std::to_string(index) +
                 " has an invalid descriptor-backed path";
        return false;
      }
      FileDescriptor descriptor(openat(
          file.directory_fd, file.path.c_str(),
          O_RDONLY | O_CLOEXEC | O_NOFOLLOW));
      if (descriptor.get() < 0) {
        *error = "open descriptor-backed storage file " +
                 std::to_string(index) + " failed: " + std::strerror(errno);
        return false;
      }
      struct stat file_stat{};
      if (fstat(descriptor.get(), &file_stat) != 0 ||
          !S_ISREG(file_stat.st_mode) || file_stat.st_size < 0 ||
          static_cast<size_t>(file_stat.st_size) != file.size) {
        *error = "descriptor-backed restore storage file " +
                 std::to_string(index) +
...
bool NixlTransfer(nixlAgent *agent, const std::string &agent_name,
  ...
  NixlRequestGuard request_guard(agent, request);
  status = agent->postXferReq(request);
  if (status == NIXL_IN_PROG) {
    request_guard.MarkInProgress();
  }
  ...
  while (status == NIXL_IN_PROG) {
    ...
    if (!cancellation_requested &&
      ...
      // The pinned NIXL POSIX backend cannot safely release an in-progress AIO
      // request: callbacks may still reference the request, registered buffer,
      // and file descriptor. Cancellation therefore records intent but keeps
      // polling until the request is terminal before releasing its handle.
    }
  ...
  const nixl_status_t release_status = request_guard.ReleaseTerminal();
  if (cancellation_requested) {
...
bool TransferPipeline(const std::vector<TransferChunk> &chunks,
                      ...
                      storage::ContentDigest *digest, TransferMetrics *metrics,
                      TransferCancellation *cancellation, std::string *error) {
  bool success = true;
  ...
  if (operation == TransferOperation::kRestore) {
    for (const auto &chunk : chunks) {
      ...
      if (!digest->Update(slot->data(), chunk.size, &digest_error)) {
        *error = "digest storage read at logical offset " +
                 std::to_string(chunk.logical_offset) + " failed: " +
                 digest_error;
        success = false;
        if (cancellation != nullptr) {
          cancellation->Cancel();
    ...
    for (; success && completed < next_chunk; ++completed) {
      ...
      std::string digest_error;
      if (!digest->Update(slot->data(), chunk.size, &digest_error)) {
        *error = "digest CUDA checkpoint data at logical offset " +
                 std::to_string(chunk.logical_offset) + " failed: " +
                 digest_error;
        success = false;
        if (cancellation != nullptr) {
  ...
  *metrics = {};
  ...
  storage::ContentDigest digest;
  {
    ...
    success = TransferPipeline(chunks, files, &slots, agent.get(), agent_name,
                               device_ptr, stream, operation, &digest, metrics,
                               cancellation, error);
    if (success) {
  ...
  if (success) {
    std::string digest_error;
    if (!digest.Finalize(&metrics->sha256, &digest_error)) {
      success = false;
      if (cancellation != nullptr) {
        cancellation->Cancel();
      }
