...
PageBroker owns a long-lived `pagebroker-gpu-engine` process. Before the CPU broker serves requests, this process initializes CUDA, retains one primary context per visible GPU, and allocates one CUDA host-NUMA staging ring per GPU and registers it with NIXL. Contexts and rings remain alive across checkpoint and restore sessions. Each ring uses 32 slots of 128 MiB (4 GiB of pinned memory per GPU). Eight GPUs therefore need 32 GiB of pinned memory plus runtime overhead; the full-model qualification uses a 64-GiB PageBroker container limit. This memory is shared across successive sessions rather than allocated for each target.

Staging buffers use `cuMemCreate` with `CU_MEM_LOCATION_TYPE_HOST_NUMA` and the GPU's `CU_DEVICE_ATTRIBUTE_HOST_NUMA_ID`, with host and GPU read/write access. On systems without NUMA, CUDA requires host node 0. This makes placement independent of the engine thread's CPU affinity or first touch. The driver must support host-NUMA virtual memory allocations; allocation errors fail engine initialization. Ring allocation and NIXL registration finish before the broker serves requests, so restores pay neither setup cost. The same allocator is used by the POSIX allocation-session worker.
