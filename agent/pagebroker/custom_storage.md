...
The checkpoint manifest records `cuda.customStorage: true`. Restore requires PageBroker for such an artifact and uses DirectRestore rather than copying the native payload into a staging tree. Before entering the restore namespaces, the agent binds one transaction-scoped session per captured CUDA PID. The inherited session FDs go only to `nsrestore`, not the workload or shim coordinator. The broker pins the target PID namespace and starts LOAD workers during binding. Workers with native payloads initialize CUDA while CRIU runs; empty-payload workers defer CUDA initialization until their first operation so they do not contend with model-worker restoration. On the first native operation it resolves the recreated target within the pinned namespace or its descendants and supplies its host PID to the worker. No target operation occurs before that binding. Native preparation is serial, LOAD transfers overlap subsequent preparation, and every transfer must finish before any process is completed and unlocked. The cuinterpose coordinator then reconstructs shared allocations and multicast state.

On cancellation, the agent closes its session FDs before sending Abort. Closing an FD starts broker-side teardown but does not mean that teardown has finished. Abort prevents new session admission and publication, then waits up to four seconds for admitted sessions to drain. The wait releases the transaction mutex; each session signals completion after its worker cleanup. Only then may Abort remove staging files. If a worker remains stuck, Abort returns a conflict and leaves those files intact; a later Abort can complete after the worker exits.
...
The qualification scope is intentionally one trusted, quiescent target. Its visible GPU set must match the worker's, including GPUs with no payload in parent processes. On restore, the worker reads the existing native manifest and applies the source/destination UUID map to retain only the primary contexts that own payload mappings. Empty manifests require no retained primary contexts. CUDA enumeration stays unchanged. `PAGEBROKER_NATIVE_SELECTED_GPU` is also available to standalone experiments. `CUDA_CHECKPOINT_JOB_FILE` must be absent unless the explicit jobfile experiment is selected. Native IPC remains outside the jobfile-free qualification: an interposed target may use the legacy-memory adapter only after cuinterpose has removed its managed shared mappings. Private tests allocate through both `cuMemAlloc` and nonexportable `cuMemCreate`; using the former does not itself create legacy IPC.

## Lifecycle

The executable takes a target PID and an existing private absolute storage directory. `--restore` selects manifest-derived context ownership. The broker uses PID `0` with this flag to initialize a worker before its target exists; after the `ready` report it sends the resolved host PID as the first input line. The worker pins that process with a pidfd before accepting native commands. Standalone workers receive a positive PID at launch and accept `save` and `load` lines on stdin:
...
A ring is reused across operations. Storage files are opened by the worker and passed as descriptors to the common transfer engine; the engine does not resolve arbitrary paths. The existing `PAGEBROKER_ALLOCATION_DIRECT_IO=1` setting enables aligned direct I/O for this adapter too.
...
Restore targets with an extent of at least 1 GiB use eight 128-MiB slots, keeping 1 GiB of pinned memory in flight. Smaller targets and SAVE use four 64-MiB slots. The larger byte window improves NFS throughput without additional transfer threads or a different NIXL backend.

## Timing and qualification

JSON reports separate initial worker/context admission, native preparation API time, transfer wall time, transfer setup, storage-request service time, CUDA-copy wait, COMPLETE, and total operation wall time. Native preparation includes internal driver reconstruction/export work; it is **not** an export-only measurement. Storage-request service times can overlap, so they must not be added to the transfer wall time.
...
The standalone multi-target harness qualifies independent, single-GPU targets. The agent integration separately exercises transaction binding, cross-node UUID remapping, CRIU, and shared-memory reconstruction. Native preparation remains serial even when transfers overlap it. Framework-specific collective behavior and cancellation of a worker blocked inside the driver require separate qualification; Abort does not force-unmap live CUDA resources.
