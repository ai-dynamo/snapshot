...
TEST(ImageIndex, CreatesPrivateAndResidualObjects)
{
	...
	std::filesystem::remove_all(directory / "parent-images");
	...
	blocked_pmap.close();
	ASSERT_EQ(criu_provider_plan_from_checkpoint(directory.c_str(), &plan), 0);
	EXPECT_EQ(plan->value.objects_size(), 3);
	EXPECT_EQ(plan->value.chunks_size(), 3);
	for (const auto& object : plan->value.objects())
		EXPECT_EQ(object.kind(), criu_provider::v1::Object::RESIDUAL);
	criu_provider_plan_destroy(plan);
	plan = nullptr;
	first.clear_blocks();
	std::ofstream raw_pmap(directory / "pagemap-7.img", std::ios::binary);
	WriteImageHeader(raw_pmap, 0x56084025);
	WriteRecord(raw_pmap, head);
	WriteRecord(raw_pmap, first);
	raw_pmap.close();
