...
/*
 * cuinterpose-coordinator: PageBroker runs this once before a CUDA
 ...
 * line per phase; PageBroker logs them. Failures go to stderr.
 */
...
static unsigned
operation_timeout_seconds(uint16_t operation)
{
  switch (operation) {
    ...
    case CUINTERPOSE_RESTORE_HOST_CARRIER:
      return carrier_timeout_seconds();
    default:
      return cuinterpose_control_timeout_seconds();
  }
}
...
connect_endpoint(const char* endpoint)
{
  ...
  if (fd < 0)
    return -1;
  if (cuinterpose_set_socket_timeouts(fd, cuinterpose_control_timeout_seconds()) != 0 ||
...
exchange(struct participant* participant, uint16_t operation, struct cuinterpose_record** records, uint32_t* count)
{
  ...
  if (fd < 0) {
    fprintf(stderr, "%s: operation %u connect failed: %s\n", participant->endpoint, operation, strerror(errno));
    goto done;
  }
  if (cuinterpose_set_socket_timeouts(fd, operation_timeout_seconds(operation)) != 0 ||
      cuinterpose_send_all(fd, &request, sizeof(request)) != 0 ||
      cuinterpose_read_all(fd, &response, sizeof(response)) != 0) {
    const int saved = errno == 0 ? EIO : errno;
    fprintf(stderr, "%s: operation %u transport failed: %s\n", participant->endpoint, operation, strerror(saved));
    goto done;
  }
  ...
  if (!strings_terminated || response.magic != CUINTERPOSE_MAGIC || response.version != CUINTERPOSE_VERSION ||
    ...
    if (strings_terminated && response.magic == CUINTERPOSE_MAGIC &&
        response.version != CUINTERPOSE_VERSION)
      fprintf(
          stderr,
          "%s: operation %u cuinterpose protocol version %u is incompatible with coordinator version %u; "
          "restart the workload with the matching shim\n",
          participant->endpoint, operation, response.version,
          CUINTERPOSE_VERSION);
    else if (strings_terminated && response.message[0] != '\0')
      fprintf(stderr, "%s: %s\n", participant->endpoint, response.message);
    goto done;
  }
  if (operation == CUINTERPOSE_IDENTIFY) {
    snprintf(participant->id, sizeof(participant->id), "%s", response.participant_id);
    participant->capabilities = response.capabilities;
...
read_state(FILE* input, struct participant** output, size_t* output_count)
{
  ...
  while (fgets(line, sizeof(line), input) != NULL) {
    ...
    if (sscanf(line, "participant %32s %u%n", id, &record_count, &consumed) != 2 ||
        consumed <= 0 || line[consumed] != '\n' || line[consumed + 1] != '\0' ||
        !cuinterpose_is_lower_hex_id(id) || record_count > CUINTERPOSE_MAX_RECORDS ||
        record_count > CUINTERPOSE_MAX_TOTAL_RECORDS - total_records ||
        count >= CUINTERPOSE_MAX_PARTICIPANTS)
      goto failed;
...
inspect(struct participant* participants, size_t count)
{
  ...
  for (index = 0; index < count; index++) {
    if (exchange(
      ...
      return -1;
    if (participants[index].count > CUINTERPOSE_MAX_TOTAL_RECORDS - total_records) {
      fprintf(stderr, "participant topology exceeds the %u-record global limit\n", CUINTERPOSE_MAX_TOTAL_RECORDS);
      return -1;
    }
    total_records += participants[index].count;
  }
  return 0;
}
...
static bool
all_support_lock_free_export(const struct participant* participants, size_t count)
{
  size_t index;

  for (index = 0; index < count; index++) {
    if ((participants[index].capabilities & CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT) == 0)
      return false;
  }
  return true;
}

/* Protocol-v3 shims embedded in existing checkpoints sampled state_lock before
 * serving EXPORT. Dispatching every importer concurrently can therefore form
 * a cycle: each participant holds its own state_lock while waiting for a peer
 * whose export listener is waiting for that peer's lock. Serialize importer
 * phases unless every restored shim advertises the lock-free path. */
static int
command_importers_compatible(struct participant* participants, size_t count, uint16_t operation)
{
  size_t index;

  if (all_support_lock_free_export(participants, count))
    return command_all_parallel(participants, count, operation);
  for (index = 0; index < count; index++) {
    struct cuinterpose_record* records = NULL;
    uint32_t record_count = 0;
...
restore_unicast(struct participant* participants, size_t count)
{
  ...
  if (command_all_parallel(participants, count, CUINTERPOSE_RESTORE_CREATORS) != 0)
    return -1;
  return command_importers_compatible(participants, count, CUINTERPOSE_RESTORE_IMPORTERS);
}
...
restore_multicast(struct participant* participants, size_t count)
{
  if (command_all_parallel(participants, count, CUINTERPOSE_RESTORE_MULTICAST_CREATORS) != 0)
    return -1;
  if (command_importers_compatible(participants, count, CUINTERPOSE_RESTORE_MULTICAST_IMPORTERS) != 0)
    return -1;
...
/* A successful legacy CUDA IPC export creates sharing state the shim cannot
 * reconstruct. Refuse before PREPARE_MULTICAST makes a destructive CUDA call. */
static int
refuse_legacy_ipc_exports(const struct participant* participants, size_t count)
{
  size_t index;
  int result = 0;

  for (index = 0; index < count; index++) {
    if (participants[index].passthrough_creations != 0) {
      fprintf(
          stderr,
          "prepare refused: participant %s (%s) created %u legacy CUDA IPC exports; "
          "CustomStorage cannot safely restore this sharing state on affected drivers; "
          "use regular PageBroker storage, disable every legacy CUDA IPC producer "
          "(including NCCL P2P) if acceptable, or use a known-fixed NVIDIA driver "
          "before checkpoint\n",
          participants[index].id, participants[index].endpoint, participants[index].passthrough_creations);
      result = -1;
    }
  }
  return result;
...
main(int argc, char** argv)
{
  ...
  if (argc == 4 && strcmp(argv[1], "--validate") == 0 && strcmp(argv[2], "--checkpoint-dir") == 0) {
    struct stat metadata;

    checkpoint_dir = argv[3];
    ...
    if (length < 0 || (size_t)length >= sizeof(state_path))
      goto done;
    state_fd = open(state_path, O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
    if (state_fd < 0 || fstat(state_fd, &metadata) != 0 || !S_ISREG(metadata.st_mode) || metadata.st_size <= 0 ||
        (uint64_t)metadata.st_size > CUINTERPOSE_MAX_STATE_BYTES || (state = fdopen(state_fd, "r")) == NULL) {
      int saved = errno == 0 ? EINVAL : errno;
      if (state_fd >= 0)
        close(state_fd);
      state_fd = -1;
      fprintf(stderr, "validate failed: %s is missing, unsafe, or outside size bounds: %s\n", state_path, strerror(saved));
      goto done;
    }
    state_fd = -1;
    if (read_state(state, &expected, &expected_count) != 0 || validate_topology(expected, expected_count, &allocations) != 0) {
      fprintf(stderr, "validate failed: checkpointed topology is invalid\n");
      goto done;
    }
    result = EXIT_SUCCESS;
    goto done;
  ...
  if (argc < 11 || (argc - 8) % 3 != 0 ||
      (strcmp(argv[1], "--prepare") != 0 && strcmp(argv[1], "--prepare-custom-storage") != 0 &&
       strcmp(argv[1], "--restore") != 0) ||
      strcmp(argv[2], "--proc-root") != 0 || strcmp(argv[4], "--checkpoint-dir") != 0 ||
      strcmp(argv[6], "--control-dir") != 0) {
    fprintf(
        stderr,
        "usage: %s --validate --checkpoint-dir PATH\n"
        "       %s (--prepare|--prepare-custom-storage|--restore) --proc-root PATH --checkpoint-dir PATH "
        "--control-dir PATH "
  ...
  prepare = strcmp(argv[1], "--restore") != 0;
  reject_legacy_ipc = strcmp(argv[1], "--prepare-custom-storage") == 0;
  proc_root = argv[3];
  ...
  if (!prepare) {
    ...
    if (state_fd < 0 || fstat(state_fd, &metadata) != 0 || !S_ISREG(metadata.st_mode) || metadata.st_size <= 0 ||
      ...
      if (state_fd >= 0)
        close(state_fd);
      fprintf(stderr, "restore failed: %s is missing, unsafe, or outside size bounds: %s\n", state_path, strerror(saved));
      return EXIT_FAILURE;
    }
    state_fd = -1;
  }
  participant_count = (size_t)(argc - 8) / 3;
  if (participant_count == 0 || participant_count > CUINTERPOSE_MAX_PARTICIPANTS) {
    fprintf(stderr, "participant count must be between 1 and %u\n", CUINTERPOSE_MAX_PARTICIPANTS);
    goto done;
  }
  ...
  if (prepare) {
    ...
    if (refuse_live_raw_imports(participants, participant_count) != 0)
      goto done;
    if (reject_legacy_ipc &&
        refuse_legacy_ipc_exports(participants, participant_count) != 0)
      goto done;
    clock_gettime(CLOCK_MONOTONIC, &phase_start);
