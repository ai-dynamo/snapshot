...
typedef CUresult(CUDAAPI* ipc_get_mem_handle_fn)(CUipcMemHandle*, CUdeviceptr);
...
struct allocation {
  ...
  struct mapping* mappings; /* canonical owner; global ranges are a non-owning address index */
  /* Checkpoint state. */
...
struct mapping {
  ...
  size_t offset;
  struct allocation* allocation;
  struct mapping* allocation_previous;
  struct mapping* allocation_next;
...
static _Atomic bool unsupported_handle_logged;
static _Atomic bool unsupported_export_handle_logged;
static _Atomic bool unsupported_import_handle_logged;
static _Atomic bool legacy_ipc_export_rejected_logged;
...
static void
link_mapping(struct allocation* allocation, struct mapping* mapping)
{
  mapping->allocation = allocation;
  mapping->allocation_previous = NULL;
  mapping->allocation_next = allocation->mappings;
  if (mapping->allocation_next != NULL)
    mapping->allocation_next->allocation_previous = mapping;
  allocation->mappings = mapping;
}

static void
unlink_mapping(struct mapping* mapping)
{
  if (mapping->allocation_previous != NULL)
    mapping->allocation_previous->allocation_next = mapping->allocation_next;
  else
    mapping->allocation->mappings = mapping->allocation_next;
  if (mapping->allocation_next != NULL)
    mapping->allocation_next->allocation_previous = mapping->allocation_previous;
}
...
settle_allocation(struct allocation* allocation)
{
  ...
  if (allocation->live_handles == 0 && allocation->mappings == NULL && !allocation->checkpointed) {
...
create_carriers(const char** error)
{
  ...
  for (index = 0; index < list.count; index++) {
    ...
    if (allocation->mappings != NULL)
      result = retain(&allocation->driver, (void*)(uintptr_t)allocation->mappings->address);
...
struct allocation_context_key {
  CUcontext context;
  CUdevice fallback_device;
};

static struct allocation_context_key
allocation_context_key(const struct allocation* allocation)
{
  struct allocation_context_key key = {
      .context = allocation->context,
      .fallback_device = CUINTERPOSE_NO_DEVICE,
  };

  if (key.context == NULL && allocation->properties.location.type == CU_MEM_LOCATION_TYPE_DEVICE)
    key.fallback_device = (CUdevice)allocation->properties.location.id;
  return key;
}

static int
compare_context_keys(struct allocation_context_key left, struct allocation_context_key right)
{
  uintptr_t left_context = (uintptr_t)left.context;
  uintptr_t right_context = (uintptr_t)right.context;

  if (left_context != right_context)
    return left_context < right_context ? -1 : 1;
  if (left.context != NULL || left.fallback_device == right.fallback_device)
    return 0;
  return left.fallback_device < right.fallback_device ? -1 : 1;
}

static int
compare_by_context(const void* left, const void* right)
{
  const struct allocation* a = *(struct allocation* const*)left;
  const struct allocation* b = *(struct allocation* const*)right;

  return compare_context_keys(allocation_context_key(a), allocation_context_key(b));
}

/* The allocations `wanted` selects, sorted by effective context so each one is entered once. */
static int
...
/* End of the run sharing an explicit context or a null context's fallback device; sum its bytes. */
static size_t
context_batch_end(const struct allocation_list* list, size_t begin, uint64_t* bytes)
{
  struct allocation_context_key key = allocation_context_key(list->items[begin]);
  size_t end = begin;

  *bytes = 0;
  while (end < list->count && compare_context_keys(allocation_context_key(list->items[end]), key) == 0) {
...
prepare(const char** error)
{
  ...
  for (begin = 0; begin < list.count; begin = end) {
    ...
    for (index = begin; index < end; index++) {
      ...
      for (mapping = allocation->mappings; mapping != NULL; mapping = mapping->allocation_next) {
...
restore_creators(const char** error)
{
  ...
  for (index = 0; index < list.count; index++) {
    if (list.items[index]->creator && list.items[index]->checkpointed)
      list.items[end++] = list.items[index];
  }
  list.count = end;
  qsort(list.items, list.count, sizeof(*list.items), compare_by_context);
  for (begin = 0; begin < list.count; begin = end) {
    ...
    for (index = begin; index < end; index++) {
      ...
      if (finish_restore(allocation, error) != 0) {
        failed = true;
        break;
      }
    }
    if (leave_context(&scope) != 0 && !failed) {
      *error = "cannot leave the allocation's CUDA context";
      failed = true;
    }
    if (failed) {
...
serve(int client)
{
  ...
  response.capabilities = CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT;
  /* EXPORT is deliberately independent of state_lock: during importer
   * restore this process may hold that lock while asking a peer for its
   * descriptor, and the peer may simultaneously ask us. Sampling a purely
   * diagnostic phase here would turn that valid cycle into a timeout. */
  if (request.operation != CUINTERPOSE_EXPORT) {
    pthread_mutex_lock(&state_lock);
...
static bool
control_peer_authorized(int client)
{
  /* The lifecycle coordinator runs in PageBroker, not in the workload's UID.
   * The pathname is therefore connectable, while SO_PEERCRED is the actual
   * authorization boundary. Same-UID peers remain necessary for descriptor
   * exchange; UID 0 is the capability-minimized PageBroker coordinator. This
   * assumes the cluster's trusted-node model and no remapped workload user
   * namespace: credentials are interpreted in this process's user namespace. */
  struct ucred peer;
  socklen_t peer_size = sizeof(peer);

  return getsockopt(client, SOL_SOCKET, SO_PEERCRED, &peer, &peer_size) == 0 && peer_size == sizeof(peer) &&
         (peer.uid == geteuid() || peer.uid == 0);
}
...
control_agent(void* unused)
{
  ...
  for (;;) {
    ...
    backoff_ms = 1;
    /* Authenticate before allocating a detached worker so an unrelated local
     * UID cannot turn the connectable socket into an unbounded thread source. */
    if (!control_peer_authorized(client)) {
      close(client);
      continue;
    }
...
start_control_endpoint(void)
{
  ...
  if (listener < 0)
    goto failed;
  /*
   * Replace only this process's pathname. PIDs in sibling or restored nested
   * PID namespaces are not meaningful to kill(pid, 0) here, so sweeping the
   * shared directory can unlink live PageBroker endpoints.
   */
  unlink(socket_path);
  if (bind(listener, (const struct sockaddr*)&address, sizeof(address)) != 0)
    goto failed;
  /* PageBroker deliberately lacks CAP_DAC_OVERRIDE and can run under a
   * different UID from the workload. The socket pathname is connectable;
   * control_connection authenticates every accepted peer with SO_PEERCRED.
   * chmod the path because fchmod(listener) does not change the socket node. */
...
static int
free_allocation_value(struct cuinterpose_key key, void* value, void* arg)
{
  struct allocation* allocation = value;
  struct mapping* mapping = allocation->mappings;

  (void)key;
  (void)arg;
  while (mapping != NULL) {
    struct mapping* next = mapping->allocation_next;
...
fork_child(void)
{
  ...
  cuinterpose_table_each(&allocations, free_allocation_value, NULL);
...
multicast_member_from_id(const uint8_t id[CUINTERPOSE_ALLOCATION_ID_SIZE], struct cuinterpose_multicast_member* member)
{
  ...
  if (allocation->mappings != NULL) {
    if (retain == NULL || retain(&member->handle, (void*)(uintptr_t)allocation->mappings->address) != CUDA_SUCCESS)
      return -1;
    ...
    CUmemGenericAllocationHandle* output, size_t size, const CUmemAllocationProp* properties, unsigned long long flags)
{
  ...
  if (properties != NULL &&
    ...
    /*
     * Reject before endpoint initialization or symbol lookup so framework
     * fallback is deterministic even when the coordinator is unavailable.
     */
    *output = 0;
    if (!atomic_exchange(&unsupported_handle_logged, true))
      warn("an allocation requested an unsupported nonzero shareable handle type; "
           "refusing it before allocation so the application can fall back");
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if ((result = cuinterpose_ensure_process_endpoint()) != CUDA_SUCCESS)
    return result;
  function = (create_fn)cuinterpose_lookup_real_symbol("cuMemCreate");
  if (function == NULL)
    return cuinterpose_unavailable();
  if (properties == NULL || properties->requestedHandleTypes == CU_MEM_HANDLE_TYPE_NONE) {
...
CUINTERPOSE_API CUresult CUDAAPI
cuIpcGetMemHandle(CUipcMemHandle* output, CUdeviceptr address)
{
  ipc_get_mem_handle_fn function;
  const char* storage_backend;
  CUresult result;

  if (output == NULL)
    return CUDA_ERROR_INVALID_VALUE;
  storage_backend = getenv(CUINTERPOSE_CUDA_STORAGE_ENV);
  if (storage_backend != NULL &&
      strcmp(storage_backend, CUINTERPOSE_CUDA_STORAGE_POSIX_CUSTOM_STORAGE) == 0) {
    /*
     * Legacy IPC export creates driver-side sharing state that CustomStorage
     * cannot restore. Refuse it before endpoint initialization or the real
     * driver call so libraries such as NCCL can fall back without leaving an
     * unrestorable export behind. Regular storage retains pass-through
     * compatibility, including when the projection is absent.
     */
    if (!atomic_exchange(&legacy_ipc_export_rejected_logged, true))
      warn("legacy CUDA IPC export is incompatible with posix-custom-storage; refusing it so the application can fall back");
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if ((result = cuinterpose_ensure_process_endpoint()) != CUDA_SUCCESS)
    return result;
  function = (ipc_get_mem_handle_fn)cuinterpose_lookup_real_symbol("cuIpcGetMemHandle");
  if (function == NULL)
    return cuinterpose_unavailable();
  result = function(output, address);
  if (result == CUDA_SUCCESS) {
    /* A CUipcMemHandle is an opaque byte value with no close operation, so
     * there is no reliable export lifetime to decrement. Keep the signal
     * cumulative for this process and fail closed for CustomStorage. */
...
cuMemMap(CUdeviceptr address, size_t size, size_t offset, CUmemGenericAllocationHandle application, unsigned long long flags)
{
  ...
  link_mapping(handle->allocation, mapping);
    ...
    void* shareable, CUmemGenericAllocationHandle application, CUmemAllocationHandleType type, unsigned long long flags)
{
  ...
  if (type != CU_MEM_HANDLE_TYPE_NONE &&
      type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    /*
     * Apply the global fail-closed contract before handle provenance is
     * inspected. Raw handles are possible through the intentional NONE/raw
     * import passthrough path; forwarding their FABRIC, mixed, or unknown
     * export would otherwise let the driver create sharing state the shim
     * cannot reconstruct.
     */
    if (!atomic_exchange(&unsupported_export_handle_logged, true))
      warn("an export requested an unsupported nonzero shareable handle type; "
           "refusing it before the driver creates uncheckpointable state");
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if ((result = cuinterpose_ensure_process_endpoint()) != CUDA_SUCCESS)
    return result;
  function = (export_fn)cuinterpose_lookup_real_symbol(
      "cuMemExportToShareableHandle");
  pthread_mutex_lock(&state_lock);
  handle = find_handle(application);
  if (handle == NULL) {
...
cuMemImportFromShareableHandle(CUmemGenericAllocationHandle* output, void* os_handle, CUmemAllocationHandleType type)
{
  ...
  if (type != CU_MEM_HANDLE_TYPE_NONE &&
      type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    /*
     * Only an exact POSIX descriptor can participate in the shim's durable
     * sharing contract. Reject FABRIC, mixed POSIX|FABRIC, and unknown nonzero
     * types before the driver can create an import that checkpoint cannot
     * reconstruct. CU_MEM_HANDLE_TYPE_NONE is left to the driver's ordinary
     * argument validation, preserving the pre-existing passthrough behavior.
     */
    *output = 0;
    if (!atomic_exchange(&unsupported_import_handle_logged, true))
      warn("an import requested an unsupported nonzero shareable handle type; "
           "refusing it before the driver creates uncheckpointable state");
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if ((result = cuinterpose_ensure_process_endpoint()) != CUDA_SUCCESS)
    return result;
  function = (import_fn)cuinterpose_lookup_real_symbol(
      "cuMemImportFromShareableHandle");
  get_properties = (properties_fn)cuinterpose_lookup_real_symbol(
      "cuMemGetAllocationPropertiesFromHandle");
  release = (release_fn)cuinterpose_lookup_real_symbol("cuMemRelease");
  if (function == NULL)
    return cuinterpose_unavailable();
  if (type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR ||
      cuinterpose_posix_read_ticket((int)(uintptr_t)os_handle, &ticket) != 0) {
    /*
     * Not one of our tickets: either a real POSIX descriptor from a process
     * without the shim or the zero handle type retained for driver validation.
     * It goes to the driver untouched, but a successful result is remembered
     * so the coordinator can refuse to checkpoint while such an import is
     * alive: native restore would give this process a private copy and silently
     * break the sharing. Unsupported nonzero types were rejected above.
     */
    ...
    if (result == CUDA_SUCCESS) {
      ...
      if (cuinterpose_table_put(&raw_imports, cuinterpose_key_u64((uint64_t)imported), (void*)1) == 0) {
        ...
        /*
         * Keep checkpoint fail-closed until the driver confirms that the
         * successfully imported but untracked allocation has been released.
         * Without this temporary count a concurrent prepare could observe no
         * raw imports and persist sharing state that cannot be reconstructed.
         */
        atomic_fetch_add(&live_raw_imports, 1);
      }
      pthread_mutex_unlock(&state_lock);
      if (!tracked) {
        *output = 0;
        if (release != NULL && release(imported) == CUDA_SUCCESS) {
          atomic_fetch_sub(&live_raw_imports, 1);
        } else {
          warn("failed to release a raw CUDA import after bookkeeping allocation failed; "
               "refusing subsequent checkpoints");
        }
        return CUDA_ERROR_OUT_OF_MEMORY;
      }
    }
  ...
  allocation->shared = true;
  if (add_handle(allocation, &logical) != 0) {
    if (allocation->live_handles == 0 && allocation->mappings == NULL) {
      cuinterpose_table_remove(&allocations, cuinterpose_key_bytes(allocation->id));
