...
serve(int client)
{
  ...
  switch (request.operation) {
    ...
    case CUINTERPOSER_EXPORT_BROKER: {
      ...
      if (current_phase != PHASE_ACTIVE || allocation == NULL || !allocation->creator || !allocation->shared) {
...
control_timeout_seconds(void)
{
  ...
  if (cached == 0) {
    ...
    cached = bounded_seconds(value, CONTROL_TIMEOUT_SECONDS_DEFAULT);
  }
  return cached;
}
...
cuMemSetAccess(CUdeviceptr address, size_t size, const CUmemAccessDesc* descriptors, size_t count)
{
  ...
  result = function != NULL ? function(address, size, descriptors, count) : cuinterposer_unavailable();
  if (result == CUDA_SUCCESS)
    ...
    size_t* granularity, const CUmulticastObjectProp* properties, CUmulticastGranularity_flags option)
{
  ...
  function_type function = (function_type)cuinterposer_lookup_real_symbol("cuMulticastGetGranularity");

  return function != NULL ? function(granularity, properties, option) : cuinterposer_unavailable();
}
