...
"""Actual core endpoints without VMM activity; provider has no GPU state."""
...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "tests/headless"))
...
if mode in ("init", "init-handle", "init-failure", "fork", "constructor"):
    ...
    def activate():
        ...
        if mode == "resolver-startup-failure":
            ...
            print("PASS actual core endpoint resolver-startup-failure: sticky refusal")
            sys.exit(0)
...
if mode not in ("fork", "constructor"):
    print(f"PASS actual core endpoint {mode}: activation, no VMM")
    sys.exit(0)
child = os.fork()
if child == 0:
    try:
...
assert inspect()["participant"] == parent
print(f"PASS actual core endpoint {mode}: parent and child, no VMM")
