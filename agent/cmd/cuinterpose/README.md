...
Snapshot's PodContract shaper copies the shim from the configured agent image
into an `emptyDir`, mounts it at `/tmp/snapshot-interposer`, and prepends
`/tmp/snapshot-interposer/libcuinterposer.so` to `LD_PRELOAD` in every
checkpoint target container. During restore, the node agent mounts the same
artifact from `/snapshot-binaries/cuinterposer` at that path. The restore
placeholder has no interposer init container and does not preload the shim;
CRIU restores the already-interposed process tree.
