...
Both artifacts require glibc 2.34 or newer, which covers Ubuntu 22.04
(glibc 2.35) and 24.04, Debian 12, and RHEL 9. Ubuntu 20.04, Debian 11, and
RHEL 8 are below the floor. The shim must also be built on glibc 2.34 or newer,
because it resolves the real `dlsym` through `dlvsym@GLIBC_2.34`. Preloading
into a workload image below the floor fails loudly — the loader reports
`version 'GLIBC_2.34' not found` and the process exits before `main` — so it
degrades every rank at container start rather than silently. The Makefile
asserts both the floor and the build base after each link, because either would
otherwise surface only at container start.
