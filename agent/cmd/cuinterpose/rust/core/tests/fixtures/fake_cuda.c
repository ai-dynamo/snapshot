    ...
    void* output, CUmemGenericAllocationHandle handle, CUmemAllocationHandleType type, unsigned long long flags)
{
  if (tracked) {
    ...
    snprintf(
        text, sizeof(text), FAKE_EXPORT_MAGIC "%d:%d:%d:%zu:%zu:%d", (int)getpid(), handles[index].allocation,
    ...
    CUmemGenericAllocationHandle* output, void* os_handle, CUmemAllocationHandleType type)
{
  if (tracked) {
    ...
    if (pread((int)(uintptr_t)os_handle, text, sizeof(text) - 1, 0) > 0 &&
        ...
        sscanf(text + strlen(FAKE_EXPORT_MAGIC), "%d:%d:%d:%zu:%zu:%d", &creator_pid, &allocation, &multicast, &size, &capacity,
               &location) != 6)
      allocation = -1;
    pthread_mutex_lock(&model_lock);
    if (creator_pid != (int)getpid() || allocation < 0 || allocation >= FAKE_MAX_ALLOCATIONS || !allocations[allocation].used) {
