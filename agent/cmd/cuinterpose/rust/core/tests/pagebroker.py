...
class ShimSessions(AllocationSessions):
    ...
    def runTest(self):
        ...
        truncated = sys.argv[1:] == ["truncated"]
        ...
        assert all(item.size == 4096 for item in manifest.extents)
        assert sorted(manifest.storage_offsets) == list(range(0, 4096 * len(manifest.extents), 4096))
        assert (staging / "allocations" / participant / "content.bin").stat().st_size == 4096 * len(manifest.extents)
        ...
        assert reply.HasField("allocation_session"), reply
        if truncated:
            allocation = self.storage / "artifact" / "allocations" / participant / "content.bin"
            allocation.write_bytes(b"short")
        ...
        if truncated:
            ...
            print("PASS PageBroker truncated LOAD: phase poisoned, no host fallback, publication refused")
            return
