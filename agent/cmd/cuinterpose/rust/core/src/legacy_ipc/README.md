...
Preloading the shim activates the adapter. Snapshot's existing launch-job policy is unchanged: jobfile-free CustomStorage qualification currently uses an explicit test launcher, not an agent-enabled production mode. Production admission, CRIU integration, and cross-node CustomStorage device remapping are separate integration work.
