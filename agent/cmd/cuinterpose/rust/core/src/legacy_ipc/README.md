...
The backend implementation and inline-ticket unit test live in this directory. The packaged process-isolated regression is `core/tests/memory_ipc.py`; it uses the existing fake-driver harness. The external integration points are the frontend/Core ABI entries, the typed driver declarations, and `State::mallocs`. The common `import_ticket` function is shared with POSIX ticket imports. When upstream native IPC and CustomStorage compose, removing the adapter does not require changing host carriers, multicast, the peer protocol, or PageBroker.

Preloading the shim activates the adapter. Snapshot omits launch-job wrapping only for the explicit `nvidia.com/cuinterpose-allocation-storage: custom-storage` mode with enabled interposition and PageBroker. Other modes retain their existing launch-job policy. The agent binds transaction-scoped PageBroker workers, captures shared bytes with host carriers, and runs native CustomStorage before CRIU. Restore recreates CPU state, loads native contents using the source-to-destination GPU UUID map, reconstructs shim sharing, and releases the workload only after both paths complete. See [native CustomStorage transfers](../../../../../../pagebroker/custom_storage.md) for worker ownership and failure handling.
