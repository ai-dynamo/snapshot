...
#ifdef CUINTERPOSE_TESTING
static _Atomic bool fail_next_table_allocation;

CUINTERPOSE_API void
cuinterpose_test_fail_next_table_allocation(void)
{
  atomic_store(&fail_next_table_allocation, true);
}
#endif
...
{
#ifdef CUINTERPOSE_TESTING
  if (atomic_exchange(&fail_next_table_allocation, false))
    return -1;
#endif
