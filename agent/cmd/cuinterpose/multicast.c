...
static struct table handles; /* logical handle -> struct multicast_handle */
static struct table objects; /* id -> struct multicast */
static struct ranges mapped; /* live mappings by address -> struct multicast_mapping */
...
{
  return table_get(&objects, key_bytes(id));
}
...
{
  struct range* range = ranges_at(&mapped, (uint64_t)address);
...
free_object(struct multicast* multicast)
{
  ...
  table_remove(&objects, key_bytes(multicast->id));
  for (index = 0; index < multicast->mapping_count; index++) {
    struct multicast_mapping* mapping = multicast->mappings[index];
    struct range* range = ranges_at(&mapped, (uint64_t)mapping->address);

    if (range != NULL && range->value == mapping)
      ranges_remove_at(&mapped, (size_t)(range - mapped.items));
    free(mapping);
  }
...
static int
collect_object(struct key key, void* value, void* arg)
{
...
list_objects(struct object_list* list)
{
  ...
  if (list->items == NULL)
    return -1;
  table_each(&objects, collect_object, list);
  return 0;
}
...
{
  return strcmp(multicast->creator_participant, ticket->creator_participant) == 0 &&
...
static int
free_handle_value(struct key key, void* value, void* arg)
{
  (void)key;
  (void)arg;
  free(value);
...
static int
free_object_value(struct key key, void* value, void* arg)
{
...
{
  table_each(&handles, free_handle_value, NULL);
  table_clear(&handles);
  table_each(&objects, free_object_value, NULL);
  table_clear(&objects);
  ranges_clear(&mapped);
}
...
cuinterpose_multicast_owns_range(CUdeviceptr address, size_t size)
{
  ...
  if (size == 0 || (uint64_t)address > UINT64_MAX - size)
    return false;
  (void)ranges_cover(&mapped, (uint64_t)address, (uint64_t)address + size, &first, &last);
  return first != last;
}
...
cuinterpose_multicast_release(CUmemGenericAllocationHandle logical)
{
  ...
  table_remove(&handles, key_u64((uint64_t)logical));
  free(handle);
  multicast->live_handles--;
  settle(multicast);
  return CUDA_SUCCESS;
    ...
    CUdeviceptr address, size_t size, size_t offset, CUmemGenericAllocationHandle logical, unsigned long long flags)
{
  ...
  switch (ranges_insert(&mapped, (uint64_t)address, (uint64_t)address + size, mapping)) {
    case 1:
      free(mapping);
      return CUDA_ERROR_INVALID_VALUE;
    case -1:
  ...
  multicast = handle->multicast;
  slot = array_push((void**)&multicast->mappings, &multicast->mapping_count, sizeof(*multicast->mappings));
  if (slot == NULL) {
    struct range* range = ranges_at(&mapped, (uint64_t)address);
...
cuinterpose_multicast_unmap(CUdeviceptr address, size_t size)
{
  ...
  if (unmap == NULL)
    return cuinterpose_unavailable();
  if (ranges_cover(&mapped, (uint64_t)address, (uint64_t)address + size, &first, &last) != 0 ||
      first == last)
    return CUDA_ERROR_INVALID_VALUE;
  ...
  for (index = last; index > first; index--) {
    ...
    ranges_remove_at(&mapped, index - 1);
    ...
    void* shareable, CUmemGenericAllocationHandle logical, CUmemAllocationHandleType type, unsigned long long flags)
{
  ...
  if (multicast->creator && !cuinterpose_export_cache_has(multicast->id)) {
    ...
    if (result != CUDA_SUCCESS)
      return result;
    if (cuinterpose_export_cache_put(multicast->id, real_fd) != 0) {
      close(real_fd);
      return CUDA_ERROR_OUT_OF_MEMORY;
    }
  }
...
cuinterpose_multicast_import(CUmemGenericAllocationHandle* output, const struct cuinterpose_posix_ticket* ticket)
{
  ...
  if (multicast == NULL) {
    ...
    if (multicast == NULL ||
        table_put(&objects, key_bytes(ticket->allocation_id), multicast) != 0) {
      ops.release_state_lock();
      (void)release(driver);
      free(multicast);
      return CUDA_ERROR_OUT_OF_MEMORY;
...
static int
count_records(struct key key, void* value, void* arg)
{
...
cuinterpose_multicast_record_count(void)
{
  ...
  table_each(&objects, count_records, &total);
  return total;
}
...
cuinterpose_multicast_write_records(struct cuinterpose_record* records, size_t count, const char** error)
{
  ...
  CUINTERPOSE_CLEANUP(free_ptr) struct multicast** listed_objects = NULL;
  size_t written = 0;
  size_t index;
...
cuinterpose_multicast_prepare(const char** error)
{
  ...
  for (index = 0; index < list.count; index++) {
    ...
    for (item = 0; item < multicast->mapping_count; item++) {
      ...
      range = ranges_at(&mapped, (uint64_t)mapping->address);
      if (range != NULL && range->value == mapping)
        ranges_remove_at(&mapped, (size_t)(range - mapped.items));
  ...
  for (index = 0; index < list.count; index++) {
    ...
    if (multicast->shared &&
        (export_handle(&real_fd, driver, CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR, 0) != CUDA_SUCCESS ||
         cuinterpose_export_cache_put(multicast->id, real_fd) != 0)) {
      if (real_fd >= 0)
        close(real_fd);
      (void)cuinterpose_leave_context(&scope);
      free(list.items);
...
/* RESTORE_MULTICAST_BINDINGS: bind the members again, map the object at the
 * old addresses with the old access, and check nothing is left checkpointed. */
int
cuinterpose_multicast_restore_bindings(const char** error)
{
  map_fn map = (map_fn)cuinterpose_lookup_real_symbol("cuMemMap");
  access_fn set_access = (access_fn)cuinterpose_lookup_real_symbol("cuMemSetAccess");
  struct object_list list;
  ...
  for (index = 0; index < list.count; index++) {
    ...
    for (item = 0; item < multicast->mapping_count; item++) {
      ...
      if (ranges_insert(&mapped, (uint64_t)mapping->address, (uint64_t)mapping->address + mapping->size,
                                    mapping) != 0) {
        (void)cuinterpose_leave_context(&scope);
        free(list.items);
...
cuMulticastCreate(CUmemGenericAllocationHandle* output, const CUmulticastObjectProp* properties)
{
  ...
  if (properties == NULL || properties->handleTypes != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    /* Same rule as cuMemCreate: private resources stay native; unsupported
     * exportable resources run normally but make checkpoint preflight fail. */
    result = function(&driver, properties);
    result = passthrough_handle(result, driver, output);
    if (result == CUDA_SUCCESS && properties != NULL && properties->handleTypes != 0)
      cuinterpose_note_unsupported_exportable_creation(properties->handleTypes);
    return result;
  }
  /*
  ...
  if (multicast == NULL || random_bytes(multicast->id, sizeof(multicast->id)) != 0 ||
      table_put(&objects, key_bytes(multicast->id), multicast) != 0) {
    ops.release_state_lock();
    if (release != NULL)
      (void)release(driver);
    free(multicast);
...
cuMulticastAddDevice(CUmemGenericAllocationHandle application, CUdevice device)
{
  ...
  multicast = handle->multicast;
  slot = array_push((void**)&multicast->devices, &multicast->device_count, sizeof(*multicast->devices));
  if (slot == NULL) {
    ops.release_state_lock();
    return CUDA_ERROR_OUT_OF_MEMORY;
  }
    ...
    unsigned long long flags, uint8_t kind)
{
  ...
  multicast = handle->multicast;
  slot = array_push((void**)&multicast->bindings, &multicast->binding_count, sizeof(*multicast->bindings));
  if (slot == NULL) {
    unbind_fn unbind = (unbind_fn)cuinterpose_lookup_real_symbol("cuMulticastUnbind");
