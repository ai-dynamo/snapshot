...
static atomic_bool unsupported_handle_logged;
...
cuMulticastCreate(CUmemGenericAllocationHandle* output, const CUmulticastObjectProp* properties)
{
  ...
  if (properties->handleTypes != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    /* Multicast sharing is reconstructable only for the exact POSIX-fd mode. */
    *output = 0;
    if (!atomic_exchange(&unsupported_handle_logged, true))
      fprintf(stderr, "cuinterpose: a multicast object requested an unsupported shareable handle type; "
                      "refusing it before allocation so the application can fall back\n");
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if ((result = cuinterpose_ensure_process_endpoint()) != CUDA_SUCCESS)
    return result;
  function = (create_fn)cuinterpose_lookup_real_symbol("cuMulticastCreate");
