...
{
  context_device_fn get_device = (context_device_fn)cuinterposer_lookup_real_symbol("cuCtxGetDevice");
    ...
    CUmemGenericAllocationHandle driver, const struct multicast_binding* binding, CUmemGenericAllocationHandle member)
{
  ...
  if (binding->api_version == 2) {
    bind_mem_v2_fn function = (bind_mem_v2_fn)cuinterposer_lookup_real_symbol("cuMulticastBindMem_v2");
    if (function == NULL)
...
#endif
  {
    bind_mem_fn function = (bind_mem_fn)cuinterposer_lookup_real_symbol("cuMulticastBindMem");
    if (function == NULL)
      return cuinterposer_unavailable();
    operations.release_state_lock();
...
#if CUDA_VERSION >= 13010
  if (binding->api_version == 2) {
    bind_address_v2_fn function = (bind_address_v2_fn)cuinterposer_lookup_real_symbol("cuMulticastBindAddr_v2");
    if (function == NULL)
      return cuinterposer_unavailable();
    operations.release_state_lock();
    result = function(
...
#endif
  {
    bind_address_fn function = (bind_address_fn)cuinterposer_lookup_real_symbol("cuMulticastBindAddr");
    if (function == NULL)
      return cuinterposer_unavailable();
    operations.release_state_lock();
...
{
  /*
   * CUDA may round a multicast object's physical capacity above the size
   * passed to cuMulticastCreate. Record the largest range the driver actually
   * accepts, but retain the original properties for export identity and replay.
   */
...
cuinterposer_multicast_release(CUmemGenericAllocationHandle logical)
{
  ...
  if (!driver_used(handle, handle->driver))
    result = release == NULL ? cuinterposer_unavailable() : release(handle->driver);
  if (result == CUDA_SUCCESS)
    handle->live = false;
  return result;
}
...
cuinterposer_multicast_retain(CUmemGenericAllocationHandle* output, void* address)
{
  ...
  if (result != CUDA_SUCCESS)
    return result;
  if (existing != 0) {
    result = release == NULL ? cuinterposer_unavailable() : release(driver);
    if (result != CUDA_SUCCESS)
      return result;
    driver = existing;
  }
    ...
    CUdeviceptr address, size_t size, size_t offset, CUmemGenericAllocationHandle logical, unsigned long long flags)
{
  ...
  if (mapping == NULL)
    return CUDA_ERROR_OUT_OF_MEMORY;
  /*
   * Multicast map waits for the complete team. Keeping state_lock held here
   * prevents the creator's control thread from exporting the driver handle
   * that the other ranks need to enter the same call.
   */
  driver = handle->driver;
  operations.release_state_lock();
  result = map == NULL ? cuinterposer_unavailable() : map(address, size, offset, driver, flags);
  operations.acquire_state_lock();
  if (result != CUDA_SUCCESS) {
    free(mapping);
    return result;
  ...
  observe_extent(handle->multicast, offset, size);
  return CUDA_SUCCESS;
}
...
cuinterposer_multicast_unmap(CUdeviceptr address, size_t size)
{
  ...
  if (mapping == NULL)
    return CUDA_ERROR_INVALID_VALUE;
  result = unmap == NULL ? cuinterposer_unavailable() : unmap(address, size);
  if (result == CUDA_SUCCESS)
...
cuinterposer_multicast_set_access(CUdeviceptr address, size_t size, const CUmemAccessDesc* descriptors, size_t count)
{
  ...
  result = set_access == NULL ? cuinterposer_unavailable() : set_access(address, size, descriptors, count);
  if (result == CUDA_SUCCESS) {
...
{
  properties_fn get_properties = (properties_fn)cuinterposer_lookup_real_symbol("cuMemGetAllocationPropertiesFromHandle");
  struct multicast_handle* handle = find_handle(logical);
  ...
  if (handle->driver == 0)
    return CUDA_ERROR_NOT_READY;
  return get_properties == NULL ? cuinterposer_unavailable() : get_properties(properties, handle->driver);
}

CUresult
cuinterposer_multicast_create(CUmemGenericAllocationHandle* output, const CUmulticastObjectProp* properties)
{
  create_fn create = (create_fn)cuinterposer_lookup_real_symbol("cuMulticastCreate");
  release_fn release = (release_fn)cuinterposer_lookup_real_symbol("cuMemRelease");
  struct multicast* multicast;
  CUmemGenericAllocationHandle driver = 0;
  CUmemGenericAllocationHandle logical;
   ...
   */
  operations.release_state_lock();
  result = create == NULL ? cuinterposer_unavailable() : create(&driver, properties);
  operations.acquire_state_lock();
  if (result != CUDA_SUCCESS)
    return result;
...
{
  add_device_fn add_device = (add_device_fn)cuinterposer_lookup_real_symbol("cuMulticastAddDevice");
  struct multicast_handle* handle = find_handle(logical);
    ...
    return CUDA_ERROR_OUT_OF_MEMORY;
  {
    ...
    operations.release_state_lock();
    result = add_device == NULL ? cuinterposer_unavailable() : add_device(driver, device);
    operations.acquire_state_lock();
    ...
    CUmemGenericAllocationHandle memory, size_t memory_offset, size_t size, unsigned long long flags)
{
  ...
  observe_extent(handle->multicast, multicast_offset, size);
  return CUDA_SUCCESS;
}
...
{
  unbind_fn unbind = (unbind_fn)cuinterposer_lookup_real_symbol("cuMulticastUnbind");
  struct multicast_handle* handle = find_handle(logical);
  struct multicast_binding* binding;
  CUresult result;
  ...
  if (binding == NULL)
    return CUDA_ERROR_NOT_SUPPORTED;
  result = unbind == NULL ? cuinterposer_unavailable() : unbind(handle->driver, device, offset, size);
  if (result != CUDA_SUCCESS)
    return result;
  binding->bound = false;
  return CUDA_SUCCESS;
