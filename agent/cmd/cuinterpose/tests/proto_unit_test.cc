...
TEST(Util, HeaderRoundTripWithDescriptor) {
  ...
  out.capabilities = CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT;
  ...
  EXPECT_EQ(in.capabilities, CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT);
...
TEST(Util, LegacyProtocolV3CapabilityFixtureUsesByte208) {
  std::array<uint8_t, sizeof(struct cuinterpose_header)> wire{};
  // Existing protocol-v3 shims emit zeroes throughout the formerly reserved
  // suffix. Pin the little-endian Linux ABI prefix as well as the capability
  // byte so future field movement cannot silently break saved workloads.
  wire[0] = 0x4d;
  wire[1] = 0x4d;
  wire[2] = 0x56;
  wire[3] = 0x44;
  ...
  std::memcpy(&header, wire.data(), wire.size());
  EXPECT_EQ(header.magic, CUINTERPOSE_MAGIC);
  EXPECT_EQ(header.version, CUINTERPOSE_VERSION);
  EXPECT_EQ(header.capabilities, 0u);

  wire[208] = CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT;
  std::memcpy(&header, wire.data(), wire.size());
  EXPECT_EQ(header.capabilities, CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT);
}
