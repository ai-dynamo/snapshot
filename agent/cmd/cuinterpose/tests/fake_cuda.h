...
enum fake_result {
  ...
  FAKE_RESULT_IPC_GET_MEM_HANDLE,
  FAKE_RESULT_RELEASE,
  FAKE_RESULT_RETAIN,
  FAKE_RESULT_MAP,
  FAKE_RESULT_UNMAP,
...
/* cuMemImportFromShareableHandle calls received by the fake driver. */
int fakeImportCalls(void);
int fakeMappedCount(void);
int fakeIsMapped(CUdeviceptr address);
int fakeUnmapCalls(void);
...
/* Fail the Nth subsequent call to the named entry point. N must be positive. */
void fakeFailOnNthCall(const char* function, unsigned int n);
/* The fake's notion of the current context, changed by cuCtxSetCurrent. */
CUcontext fakeCurrentContext(void);
/* Calls that made a particular context current since the counter reset. */
int fakeContextSetCalls(CUcontext context);
void fakeResetContextSetCalls(void);
...
/* Staging-range reservations made while a device's fake primary context was
 * current. Carrier tests use this to observe effective-context batches without
 * depending on unrelated lifecycle context entry counts. */
int fakeAddressReserveCallsForPrimaryDevice(CUdevice device);
