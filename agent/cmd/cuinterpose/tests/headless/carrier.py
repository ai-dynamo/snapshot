...
from support import driver, props, stats as read_stats
...
def main():
    ...
    # Resolver-only activity must refuse a poisoned generation as well. The
    # driver fixture exposes this unwrapped function without invoking it.
    cuda.cuGetProcAddress.argtypes = [
        c.c_char_p, c.POINTER(c.c_void_p), c.c_int, c.c_uint64,
    ]
    function = c.c_void_p(0xAAAA)
    assert cuda.cuGetProcAddress(b"cuCtxGetCurrent", c.byref(function), 13010, 0) == 600
    assert function.value is None, "failed generation published a driver pointer"
    for fd in tickets:
        os.close(fd)
