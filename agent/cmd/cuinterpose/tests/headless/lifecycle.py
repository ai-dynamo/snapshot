...
from support import driver, props, Properties, Location, stats as read_stats
from protocol_client import LIFECYCLE, command, decode, seal_ticket

cuda = driver()
stats = partial(read_stats, cuda)
...
def main():
    ...
    if mode in ("stale-unicast", "stale-multicast"):
        handle, ticket = u64(), c.c_int(-1)
        if mode == "stale-unicast":
            assert cuda.cuMemCreate(c.byref(handle), length, c.byref(props), 0) == 0
        else:
            class Multicast(c.Structure):
                _fields_ = [("devices", c.c_uint), ("size", size),
                            ("handles", u64), ("flags", u64)]
            cuda.cuMulticastCreate.argtypes = [c.POINTER(u64), c.POINTER(Multicast)]
            assert cuda.cuMulticastCreate(
                c.byref(handle), c.byref(Multicast(2, length, 1, 0))) == 0
        assert cuda.cuMemExportToShareableHandle(c.byref(ticket), handle, 1, 0) == 0
        assert cuda.cuMemRelease(handle) == 0
        # Both explicit creator refusal and a missing endpoint are expected
        # import failures, not evidence that this process's CUDA state is bad.
        metadata = decode(os.pread(ticket.value, 4096, 4))
        metadata["endpoint"] = os.environ["SNAPSHOT_CONTROL_DIR"] + "/absent.sock"
        missing = seal_ticket(metadata)
        for descriptor in (ticket.value, missing):
            imported = u64(0xAAAA)
            assert cuda.cuMemImportFromShareableHandle(c.byref(imported), descriptor, 1) == 400
            assert imported.value == 0xAAAA and stats().phase == 1
            unrelated = u64()
            assert cuda.cuMemCreate(c.byref(unrelated), length, c.byref(props), 0) == 0
            assert cuda.cuMemRelease(unrelated) == 0
            os.close(descriptor)
        print("PASS recoverable peer refusal and unavailable creator:", mode)
        return
    if mode == "no-context":
    ...
    if mode in ("raw", "unsupported"):
        ...
        assert stats().phase == 1 and cuda.fakeMappedCount() == 1
        return
    if mode == "exhaustion":
        ...
        ticket = c.c_int(-1)
        assert cuda.cuMemExportToShareableHandle(c.byref(ticket), handle, 1, 0) != 0
        assert stats().allocations == 1 and stats().handles == 1
        for fd in opened:
            os.close(fd)
        assert cuda.cuMemExportToShareableHandle(c.byref(ticket), handle, 1, 0) == 0
    ...
    if mode == "tracking":
        ...
        assert stats().handles == 2 and stats().allocations == 1
        ...
        for _ in range(100):
            assert cuda.cuMemCreate(c.byref(handle), length, c.byref(props), 0) == 0
            assert cuda.cuMemRelease(handle) == 0
        assert stats().allocations == 0 and stats().handles == 0
        return
    if mode == "access":
    ...
    if shared:
        ...
        if mode != "no-context":
            ...
            assert stats().allocations == 1 and stats().handles == 2
        os.close(ticket.value)
        if mode == "no-context":
