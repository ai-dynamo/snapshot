...
from support import driver, props, Stats, stats as read_stats
...
def disconnected_readers():
    ...
    while not cuda.rpc_copy_entered() and time.monotonic() < deadline:
        time.sleep(0.001)
    assert cuda.rpc_copy_entered()
    # The copy gate keeps the worker from replying until these clients have
    # closed. The final handshake is a FIFO barrier, not a timing assumption.
    for operation in ("handshake", "inspect"):
        connection = request(path, operation, identity)
        connection.shutdown(socket.SHUT_RDWR)
        connection.close()
    barrier = request(path, "handshake")
    cuda.rpc_release_copy()
    reply(saving)
    reply(barrier)
    assert stats().phase == 2, "read-only reply failure poisoned the generation"
    for operation in LIFECYCLE[2:]:
        reply(request(path, operation, identity))
    unrelated = c.c_uint64()
    assert cuda.cuMemCreate(c.byref(unrelated), 4096, c.byref(props), 0) == 0
    assert cuda.cuMemRelease(unrelated) == 0
    os.close(ticket.value)
    print("PASS RPC closed handshake/inspection clients leave CUDA state usable")
