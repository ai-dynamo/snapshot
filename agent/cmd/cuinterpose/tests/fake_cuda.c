...
#define FAKE_MAX_PRIMARY_DEVICES 64
#define FAKE_MAX_CONTEXT_COUNTERS 64

struct fake_context_counter {
  CUcontext context;
  int calls;
};
...
static int address_reserve_calls_by_primary_device[FAKE_MAX_PRIMARY_DEVICES];
static struct fake_context_counter context_set_calls[FAKE_MAX_CONTEXT_COUNTERS];
static char fail_next[64];
static unsigned int fail_calls_remaining;
...
static _Atomic int import_calls;
...
fakeResetModel(void)
{
  ...
  memset(address_reserve_calls_by_primary_device, 0,
         sizeof(address_reserve_calls_by_primary_device));
  memset(context_set_calls, 0, sizeof(context_set_calls));
  export_calls = 0;
  atomic_store(&import_calls, 0);
  access_calls = 0;
  unmap_calls = 0;
  copied_to_host = 0;
  copied_to_device = 0;
  fail_next[0] = '\0';
  fail_calls_remaining = 0;
...
int
fakeImportCalls(void)
{
  return atomic_load(&import_calls);
}
...
int
fakeIsMapped(CUdeviceptr address)
{
  int index;
  int found = 0;
...
CUresult CUDAAPI
fakeCuIpcGetMemHandle(CUipcMemHandle* output, CUdeviceptr address)
{
  record("cuIpcGetMemHandle");
  last.pointer = output;
  last.address = address;
  if (!tracked)
    return (CUresult)FAKE_RESULT_IPC_GET_MEM_HANDLE;
  if (should_fail("cuIpcGetMemHandle"))
    return CUDA_ERROR_UNKNOWN;
  if (output == NULL)
    return CUDA_ERROR_INVALID_VALUE;
...
{
  return fakeCuIpcGetMemHandle(output, address);
}
...
{
  atomic_fetch_add(&import_calls, 1);
  if (tracked) {
    char text[128] = {0};
    int allocation = -1;
    int multicast = 0;
...
{
  fakeFailOnNthCall(function, 1);
}

void
fakeFailOnNthCall(const char* function, unsigned int n)
{
  pthread_mutex_lock(&model_lock);
  snprintf(fail_next, sizeof(fail_next), "%s", function);
  fail_calls_remaining = n;
  pthread_mutex_unlock(&model_lock);
}
...
should_fail(const char* function)
{
  ...
  pthread_mutex_lock(&model_lock);
  if (fail_calls_remaining != 0 && strcmp(fail_next, function) == 0) {
    fail_calls_remaining--;
    if (fail_calls_remaining == 0) {
      fail_next[0] = '\0';
      fail = 1;
    }
  }
...
cuCtxSetCurrent(CUcontext context)
{
  ...
  pthread_mutex_lock(&model_lock);
  for (index = 0; index < FAKE_MAX_CONTEXT_COUNTERS; index++) {
    if (context_set_calls[index].calls != 0 && context_set_calls[index].context != context)
      continue;
    context_set_calls[index].context = context;
    context_set_calls[index].calls++;
    break;
  }
  pthread_mutex_unlock(&model_lock);
  return CUDA_SUCCESS;
...
int
fakeContextSetCalls(CUcontext context)
{
  int calls = 0;
  int index;
  ...
  for (index = 0; index < FAKE_MAX_CONTEXT_COUNTERS; index++) {
    if (context_set_calls[index].calls != 0 && context_set_calls[index].context == context) {
      calls = context_set_calls[index].calls;
      break;
    }
  }
  pthread_mutex_unlock(&model_lock);
...
cuMemAddressReserve(CUdeviceptr* address, size_t size, size_t alignment, CUdeviceptr fixed, unsigned long long flags)
{
  ...
  if (should_fail("cuMemAddressReserve"))
    return CUDA_ERROR_UNKNOWN;
  pthread_mutex_lock(&model_lock);
  uintptr_t context = (uintptr_t)current_context;
  if (context >= 0x100 &&
      context < 0x100 + FAKE_MAX_PRIMARY_DEVICES)
    address_reserve_calls_by_primary_device[context - 0x100]++;
...
fakePrimaryContextsHeld(void)
{
  ...
  pthread_mutex_lock(&model_lock);
  held = primary_contexts_held;
  pthread_mutex_unlock(&model_lock);
  return held;
}

int
fakeAddressReserveCallsForPrimaryDevice(CUdevice device)
{
  int calls = 0;

  if (device < 0 || device >= FAKE_MAX_PRIMARY_DEVICES)
    return 0;
  pthread_mutex_lock(&model_lock);
  calls = address_reserve_calls_by_primary_device[device];
  pthread_mutex_unlock(&model_lock);
  return calls;
}
...
fakeOriginalForVersion(const char* symbol, int version)
{
  static const struct {
    ...
    void* function;
      ...
      {"cuIpcGetMemHandle", (void*)&fakeCuIpcGetMemHandle},
