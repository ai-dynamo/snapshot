...
class Participant {
 public:
  ...
  uint32_t unsupported_exportable_creations = 0;
 ...
 private:
  ...
  void handle(int client) {
    ...
    if (cuinterpose_receive_header(client, &request, &passed) == 0) {
      ...
      reply.unsupported_exportable_creations = unsupported_exportable_creations;
      reply.phase = phase;
      bool failing = false;
      ...
      if (failing) {
        ...
        if (cuinterpose_send_header(client, &reply, -1) == 0 && !records.empty())
          send_all(client, records.data(), reply.payload_size);
      } else {
        cuinterpose_send_header(client, &reply, -1);
      }
    }
...
TEST_F(Coordinator, PrepareRefusesLiveRawImports) {
  ...
  // Nothing destructive happened: no prepare operation was sent.
  for (auto* p : {&a, &b}) {
    for (uint16_t op : p->operations()) {
      EXPECT_NE(op, CUINTERPOSE_PREPARE_MULTICAST);
      EXPECT_NE(op, CUINTERPOSE_PREPARE_UNICAST);
    }
  }
}

TEST_F(Coordinator, PrepareRefusesUnsupportedExportableCreations) {
  Participant a(control, 1, kIdA);
  a.unsupported_exportable_creations = 2;

  Outcome run = run_coordinator(args("--prepare", {1}));
  EXPECT_NE(run.status, 0);
  EXPECT_NE(run.err.find("created 2 CUDA resources with unsupported exportable handle types"), std::string::npos)
      << run.err;
  EXPECT_FALSE(state_exists());
  for (uint16_t op : a.operations()) {
    EXPECT_NE(op, CUINTERPOSE_PREPARE_MULTICAST);
    EXPECT_NE(op, CUINTERPOSE_SAVE_ALLOCATIONS);
    EXPECT_NE(op, CUINTERPOSE_PREPARE_UNICAST);
  }
}
...
TEST_F(Coordinator, PrepareStopsAtTheFirstFailingParticipant) {
  ...
  EXPECT_FALSE(state_exists());
  for (uint16_t op : a.operations())
    EXPECT_NE(op, CUINTERPOSE_PREPARE_UNICAST) << "PREPARE_UNICAST must not run after a failure";
}
...
TEST_F(Coordinator, RestoreRunsPhasesWithBarriers) {
  ...
  // not receive RESTORE_MULTICAST_BINDINGS: binding before every device is
  ...
  a.before_reply = [&](uint16_t op) {
    if (op == CUINTERPOSE_RESTORE_MULTICAST_BINDINGS) {
  ...
  std::vector<uint16_t> want = {CUINTERPOSE_HANDSHAKE,
                                ...
                                CUINTERPOSE_RESTORE_MULTICAST_BINDINGS,
                                CUINTERPOSE_HANDSHAKE,
                                CUINTERPOSE_INSPECT};
