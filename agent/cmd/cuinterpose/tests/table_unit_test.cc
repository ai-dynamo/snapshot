...
class ScopedProcessAlarm {
 public:
  explicit ScopedProcessAlarm(unsigned seconds) { alarm(seconds); }
  ~ScopedProcessAlarm() { alarm(0); }
};

class ScopedWorkers {
 public:
  ScopedWorkers(std::atomic<bool>& stop, std::vector<std::thread>& workers)
      : stop_(stop), workers_(workers) {}
  ~ScopedWorkers() {
    stop_ = true;
...
TEST_F(ExportCache, ConcurrentServeAndDropIsSafe) {
  ASSERT_EQ(cuinterpose_export_cache_put(id, auth, memfd("gpu")), 0);
  ScopedProcessAlarm process_timeout(10);
  std::atomic<bool> stop{false};
  std::atomic<bool> start{false};
  std::atomic<int> ready{0};
  std::atomic<int> served{0};
  std::atomic<int> concurrent_attempts{0};
  std::atomic<int> initial_failures{0};
  auto serve_once = [&] {
    int dup = -1;
    const char* reason = nullptr;
    if (cuinterpose_export_cache_begin(id, auth, &dup, &reason) != 0) return false;
    char buffer[4] = {};
    EXPECT_EQ(pread(dup, buffer, 3, 0), 3);
    EXPECT_STREQ(buffer, "gpu");
    close(dup);
  ...
  int put_error = 0;
  {
    ...
    ScopedWorkers scoped_workers(stop, workers);
    for (int i = 0; i < 4; i++) {
      workers.emplace_back([&] {
        if (!serve_once()) initial_failures++;
        ready++;
        while (!start && !stop) std::this_thread::yield();
        if (stop) return;
        do {
          (void)serve_once();
          concurrent_attempts++;
    ...
    for (int round = 0; round < 200; round++) {
      cuinterpose_export_cache_drop(id);
      put_error = cuinterpose_export_cache_put(id, auth, memfd("gpu"));
      if (put_error != 0) break;
    }
    stop = true;
  }
  ASSERT_EQ(put_error, 0);
  EXPECT_EQ(initial_failures.load(), 0);
  EXPECT_GE(served.load(), 4);
  EXPECT_GE(concurrent_attempts.load(), 4);
}
