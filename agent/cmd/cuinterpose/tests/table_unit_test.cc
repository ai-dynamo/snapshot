...
TEST(Ranges, GrowAndShrink) {
  cuinterpose_ranges ranges{};
  for (uint64_t i = 0; i < 5000; i++) {
    ASSERT_EQ(cuinterpose_ranges_insert(&ranges, i * 0x1000, i * 0x1000 + 0x800, nullptr), 0);
  }
  EXPECT_EQ(ranges.count, 5000u);
  // Remove from the front until empty: capacity shrinks along the way.
  size_t peak = ranges.capacity;
  while (ranges.count > 0) cuinterpose_ranges_remove_at(&ranges, 0);
  EXPECT_LT(ranges.capacity, peak);
  EXPECT_EQ(ranges.items, nullptr);
}
