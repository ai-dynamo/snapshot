...
// asked for), unbinding and rebinding by address, rejection of non-POSIX
...
// Multicast objects are reconstructable only for the exact POSIX descriptor
// mode. Unsupported requests fail before the driver can allocate an object.
TEST_F(Multicast, NonPosixObjectsFailBeforeAllocation) {
  for (unsigned type : {static_cast<unsigned>(CU_MEM_HANDLE_TYPE_NONE),
                        ...
                        1u << 30}) {
    CUmulticastObjectProp props = multicast_props(1, kMiB);
    props.handleTypes = static_cast<CUmemAllocationHandleType>(type);
    CUmemGenericAllocationHandle raw = 0xfeed;
    EXPECT_EQ(cuMulticastCreate(&raw, &props), CUDA_ERROR_NOT_SUPPORTED) << type;
    EXPECT_EQ(raw, 0u);
    EXPECT_EQ(stats().multicasts, 0u);
    EXPECT_EQ(fakeMulticastObjects(), 0) << "the driver must not receive an unsupported multicast request";
  }
}

TEST_F(Multicast, NullPropertiesFailBeforeDriverCall) {
  CUmemGenericAllocationHandle raw = 0xfeed;
  EXPECT_EQ(cuMulticastCreate(&raw, nullptr), CUDA_ERROR_INVALID_VALUE);
  EXPECT_EQ(raw, 0u);
  EXPECT_EQ(fakeMulticastObjects(), 0);
}
