...
// Unsupported exportable multicast objects run normally, but checkpoint is
// rejected before PREPARE_MULTICAST because they cannot yet be reconstructed.
TEST_F(Multicast, UnsupportedExportableObjectsPassThroughButBlockCheckpoint) {
  pid_t child = fork();
  ASSERT_GE(child, 0);
  if (child == 0) {
    CUmulticastObjectProp props = multicast_props(1, kMiB);
    ...
    result |= cuMulticastCreate(&raw, &props) != CUDA_SUCCESS;
    result |= is_logical(raw);
    result |= stats().multicasts != 0;
    result |= stats().unsupported_exportable_creations != 1;
    result |= cuMulticastAddDevice(raw, 0) != CUDA_SUCCESS;
    result |= cuMemRelease(raw) != CUDA_SUCCESS;
    Outcome prepare = coordinate("--prepare", checkpoint, {getpid()});
    result |= prepare.status == 0;
    result |= prepare.err.find("unsupported exportable handle types") == std::string::npos;
    result |= stats().phase != static_cast<uint32_t>(CUINTERPOSE_PHASE_ACTIVE);
    _exit(result);
  }
  int status = 0;
...
TEST_F(Multicast, PrepareMulticastClosesTheCachedDescriptorBeforeReleasingTheObject) {
  ...
  struct cuinterpose_header identity = request_own_shim(CUINTERPOSE_HANDSHAKE, nullptr);
  ...
  EXPECT_EQ(stats().cached_exports, 1u) << "only the unicast descriptor is still cached; PREPARE_UNICAST closes that one";
  EXPECT_EQ(fakeMulticastObjects(), 0) << "the object itself was released";
  EXPECT_EQ(fakeMulticastBindings(0), 0);
  EXPECT_EQ(fakeMappedCount(), 1) << "the member's own mapping is untouched until PREPARE_UNICAST";
