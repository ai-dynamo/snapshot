...
TEST_F(Forwarding, UnicastEntryPointsForwardArguments) {
  ...
  CUipcMemHandle ipc{};
  EXPECT_EQ(cuIpcGetMemHandle(&ipc, 0x12340000),
            static_cast<CUresult>(FAKE_RESULT_IPC_GET_MEM_HANDLE));
  EXPECT_STREQ(last().function, "cuIpcGetMemHandle");
  EXPECT_EQ(last().pointer, static_cast<void*>(&ipc));
  EXPECT_EQ(last().address, 0x12340000u);
...
TEST_F(Forwarding, DlsymOnTheDriverHandleReturnsTheShim) {
  void* cuda = dlopen("libcuda.so.1", RTLD_NOW);
  ASSERT_NE(cuda, nullptr);
  for (const char* name : {"cuMemCreate", "cuIpcGetMemHandle", "cuMemMap", "cuMemExportToShareableHandle", "cuMulticastCreate",
...
TEST_F(Forwarding, CuGetProcAddressSubstitutesAllVersions) {
  for (const char* name : {"cuMemCreate", "cuIpcGetMemHandle", "cuMemRelease", "cuMemRetainAllocationHandle", "cuMemMap", "cuMemUnmap",
