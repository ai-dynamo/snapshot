...
/* Operator-projected CUDA storage backend for this workload. */
#define CUINTERPOSE_CUDA_STORAGE_ENV "SNAPSHOT_CUDA_STORAGE"
#define CUINTERPOSE_CUDA_STORAGE_POSIX_CUSTOM_STORAGE "posix-custom-storage"
...
/* Longer timeout for lifecycle operations that map, copy, unmap, or rebuild
 * allocation state; they scale with the amount of tracked memory. */
...
#define CUINTERPOSE_MAX_TOTAL_RECORDS 32768U
#define CUINTERPOSE_MAX_PARTICIPANTS 64U
#define CUINTERPOSE_MAX_STATE_BYTES (64U * 1024U * 1024U)
...
/* Additive protocol-v3 capabilities. Older shims leave this field zero, so a
 * newer coordinator can select a safe compatibility path without rejecting a
 * checkpoint that restored the older shim's process image. */
enum cuinterpose_capability {
  CUINTERPOSE_CAPABILITY_LOCK_FREE_EXPORT = 1U << 0,
};
...
struct cuinterpose_header {
  ...
  uint32_t passthrough_creations; /* legacy field name: cumulative successful IPC exports */
  ...
  uint32_t capabilities; /* cuinterpose_capability bits; replies only */
  uint8_t reserved[44];
};
...
CUINTERPOSE_STATIC_ASSERT(
    offsetof(struct cuinterpose_header, capabilities) == 208, "cuinterpose protocol-v3 capability offset changed");
