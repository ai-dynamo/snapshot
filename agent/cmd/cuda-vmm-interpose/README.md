...
Every workload image used for checkpoint and restore must provide the launcher
at `/usr/local/bin/snapshot-cuda-vmm-launch` and the readable interposer library
at `/usr/local/lib/snapshot/libsnapshot_cuda_vmm.so`. `snapshotctl` uses these
common paths but does not inject either file into the workload image.
...
`snapshotctl` injects the interpose launcher. Single-GPU POSIX VMM
checkpointing does not require a CUDA launch-job file. Multi-GPU
...
cuda-checkpoint --launch-job \
  uv run --project agent/cmd/cuda-vmm-interpose/tests \
    pytest agent/cmd/cuda-vmm-interpose/tests/test_cucheckpoint.py -v
```
