...
exchange(struct participant* participant, uint16_t operation, struct snapshot_vmm_record** records, uint32_t* count)
{
  ...
  if (fd < 0 || snapshot_vmm_write_all(fd, &request, sizeof(request)) != 0 ||
    ...
    goto done;
  strings_terminated = snapshot_vmm_header_strings_terminated(&response);
  if (!strings_terminated || response.magic != SNAPSHOT_VMM_MAGIC || response.version != SNAPSHOT_VMM_VERSION ||
