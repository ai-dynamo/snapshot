...
bool TestPinnedMemoryCalculation() {
  ...
  return Check(transfer::CalculatePinnedBytes(
         ...
         Check(transfer::CalculatePinnedBytes(
                   ...
                   &bytes, &error),
               error) &&
         Check(bytes == 3ULL * 1024ULL * 1024ULL * 1024ULL,
               "aggregate per-target pinned bytes are wrong") &&
         Check(!transfer::CalculatePinnedBytes(
         ...
         Check(transfer::CalculateBatchPinnedBytes(
                   ...
                   &error) &&
                   bytes == transfer::kMaximumPinnedBytesPerOperation + 1,
               "aggregate batch pinned bytes were capped instead of scheduled") &&
         Check(!transfer::CalculateBatchPinnedBytes(
