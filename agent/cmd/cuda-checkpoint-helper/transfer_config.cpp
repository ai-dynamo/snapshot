...
bool BuildTransferChunks(size_t extent_size, const StorageLayout &storage,
  ...
  for (const auto &file : storage.files) {
    const bool descriptor_backed =
        file.directory_fd >= 0 || file.descriptor_fd >= 0;
    const bool valid_path = descriptor_backed
                                ? !file.path.empty() && !file.path.is_absolute() &&
                                      file.path.parent_path().empty() &&
                                      file.path.filename() != "." &&
                                      file.path.filename() != ".."
                                : !file.path.empty() && file.path.is_absolute();
    if (!valid_path || file.size == 0) {
      *error = "storage files must have a safe path and nonzero sizes";
      return false;
    }
    std::filesystem::path identity = file.path.lexically_normal();
    if (descriptor_backed)
      identity = std::to_string(file.descriptor_fd >= 0 ? file.descriptor_fd
                                                        : file.directory_fd) +
                 ":" + identity.string();
    if (!paths.insert(std::move(identity)).second) {
