...
Request TestRequest(Action action) {
  return Request{
      ...
      .expected_job_file_device = action == Action::kRestore ? 7U : 0U,
      .expected_job_file_inode = action == Action::kRestore ? 70U : 0U,
      .selected_devices =
          action == Action::kCheckpoint || action == Action::kRestore
              ? "GPU-12345678-1234-1234-1234-123456789abc"
              : "",
      .storage_relative_dir = {},
      .pinned_storage_files = {},
      .targets = {},
  };
}

Request LiveProcessRequest(pid_t pid) {
  Request request = TestRequest(Action::kRestore);
  request.pid = static_cast<uint32_t>(pid);
  std::ifstream stat("/proc/" + std::to_string(pid) + "/stat");
  std::string stat_line;
  assert(std::getline(stat, stat_line));
  const size_t closing_paren = stat_line.rfind(')');
  assert(closing_paren != std::string::npos);
  std::istringstream fields(stat_line.substr(closing_paren + 2));
  std::string field;
  for (size_t index = 0; index < 20; ++index) {
    assert(fields >> field);
    if (index == 19) {
      request.expected_start_time_ticks = std::stoull(field);
    }
  }
  std::ifstream cgroup("/proc/" + std::to_string(pid) + "/cgroup");
  std::ostringstream contents;
  contents << cgroup.rdbuf();
  request.expected_cgroup = contents.str();
  assert(!request.expected_cgroup.empty());
  return request;
}
...
void TestGoldenRequestFixture() {
  ...
  assert(parsed.expected_job_file_device == 0);
  assert(parsed.expected_job_file_inode == 0);
  assert(parsed.selected_devices ==
         "GPU-12345678-1234-1234-1234-123456789abc");
  parsed.expected_job_file_device = 7;
  parsed.expected_job_file_inode = 70;
  std::vector<unsigned char> reencoded;
  assert(EncodeRequest(parsed, &reencoded, &error));
  assert(reencoded != encoded);
  Request upgraded;
  assert(ParseRequest(reencoded.data(), reencoded.size(), &upgraded, &error));
  assert(upgraded.pid == parsed.pid && upgraded.job_file == parsed.job_file);
}
...
void TestProtocol() {
  ...
  request = TestRequest(Action::kRestore);
  request.storage_dir =
      "/pagebroker/staging/restore/transaction/cuda-custom-storage";
  assert(EncodeRequest(request, &encoded, &error));
  assert(!ParseRequest(encoded.data(), encoded.size(), &parsed, &error));
  assert(ParseRequest(encoded.data(), encoded.size(), &parsed, &error,
                      "/pagebroker/staging"));
  for (const std::string &invalid_storage : {
           std::string("/pagebroker/staging"),
           std::string("/pagebroker/staging-other/restore"),
           std::string("/pagebroker/staging/../secrets")}) {
    request.storage_dir = invalid_storage;
    assert(EncodeRequest(request, &encoded, &error));
    assert(!ParseRequest(encoded.data(), encoded.size(), &parsed, &error,
                         "/pagebroker/staging"));
...
void TestRestoreBatchProtocol() {
  ...
  first.expected_job_file_device = 7;
  first.expected_job_file_inode = 70;
  ...
  second.expected_job_file_device = 8;
  second.expected_job_file_inode = 80;
  ...
  assert(parsed.targets[0].expected_job_file_device == 7);
  assert(parsed.targets[0].expected_job_file_inode == 70);
  assert(parsed.targets[1].expected_job_file_device == 8);
  assert(parsed.targets[1].expected_job_file_inode == 80);

  batch.targets[0].storage_dir =
      "/pagebroker/staging/restore/transaction/process-nspid-123";
  batch.targets[1].storage_dir =
      "/pagebroker/staging/restore/transaction/process-nspid-124";
  assert(EncodeRequest(batch, &encoded, &error));
  assert(ParseRequest(encoded.data(), encoded.size(), &parsed, &error,
                      "/pagebroker/staging"));
  assert(parsed.targets.size() == 2);
  assert(!ParseRequest(encoded.data(), encoded.size(), &parsed, &error));
  batch.targets = {first, second};
  ...
  batch.targets[1] = second;
  batch.targets[1].job_file.clear();
  assert(EncodeRequest(batch, &encoded, &error));
  assert(!ParseRequest(encoded.data(), encoded.size(), &parsed, &error));
...
void TestProcessIdentityStates() {
  ...
  const std::filesystem::path process_dir =
      std::filesystem::path(proc_root) / std::to_string(request.pid);
  request.expected_cgroup =
      "0::/kubepods.slice/kubepods-burstable.slice/"
      "kubepods-burstable-podabc.slice/cri-containerd-def.scope\n";
  {
    std::ofstream cgroup(process_dir / "cgroup");
    cgroup << "0::/../../kubepods-burstable-podabc.slice/"
  ...
  {
    std::ofstream cgroup(process_dir / "cgroup");
    cgroup << "0::/../../kubepods-burstable-podother.slice/"
              "cri-containerd-def.scope\n";
  }
  assert(InspectProcessIdentity(request, proc_root, &error) ==
  ...
  {
    std::ofstream cgroup(process_dir / "cgroup");
    cgroup << "0::/../../cri-containerd-def.scope\n";
  }
  assert(InspectProcessIdentity(request, proc_root, &error) ==
         ProcessIdentityState::kIndeterminate);
...
void TestHealthStates() {
  OperationHealth regular_operation(std::chrono::seconds(1), "instance-a");
  ...
  OperationHealth no_operation(std::chrono::seconds(1), "instance-b");
  ...
  assert(response.output.find("\"incarnation\":\"instance-b\"") !=
         std::string::npos);
...
void TestIncarnationGeneration() {
  std::string first;
  std::string second;
  std::string error;
  assert(GenerateIncarnation(&first, &error));
  assert(GenerateIncarnation(&second, &error));
  assert(first.size() == 36 && second.size() == 36 && first != second);
  assert(first[8] == '-' && first[13] == '-' && first[18] == '-' &&
         first[23] == '-' && first[14] == '4');
}
...
void TestTerminateMatchingProcesses() {
  assert(!TerminateMatchingProcesses({}, "/proc", std::chrono::seconds(1),
                                     nullptr));
  const pid_t child = fork();
  assert(child >= 0);
  if (child == 0) {
  ...
  const Request request = LiveProcessRequest(child);
  std::thread reaper([child] {
    int status = 0;
    assert(waitpid(child, &status, 0) == child);
    assert(WIFSIGNALED(status));
  });
  std::string error;
  const bool terminated = TerminateMatchingProcesses(
      {request}, "/proc", std::chrono::seconds(2), &error);
  if (!terminated) {
    // Do not strand the child (and therefore the build step's output pipe)
    // when the assertion fails in a constrained container runtime.
    (void)kill(child, SIGKILL);
  }
  reaper.join();
  const bool pidfd_supported = terminated;
  if (!pidfd_supported) {
    const bool pidfd_unavailable =
        error.find("pidfd_open target") != std::string::npos &&
        error.find(std::strerror(ENOSYS)) != std::string::npos;
    if (pidfd_unavailable) {
      std::fprintf(stderr,
                   "skipping pidfd-dependent termination checks: %s\n",
                   error.c_str());
    } else {
      std::fprintf(stderr, "TerminateMatchingProcesses failed: %s\n",
                   error.c_str());
    }
    assert(pidfd_unavailable);
  }

  Request reused = LiveProcessRequest(getpid());
  ++reused.expected_start_time_ticks;
  assert(TerminateMatchingProcesses({reused}, "/proc",
                                    std::chrono::milliseconds(20), &error));

  const auto proc_root = std::filesystem::temp_directory_path() /
                         ("daemon-termination-test-" +
                          std::to_string(static_cast<unsigned long>(getpid())));
  std::filesystem::remove_all(proc_root);
  std::filesystem::create_directories(proc_root / "42");
  Request indeterminate = TestRequest(Action::kRestore);
  indeterminate.pid = 42;
  assert(!TerminateMatchingProcesses({indeterminate}, proc_root.string(),
                                     std::chrono::milliseconds(20), &error));
  assert(error.find("cannot safely terminate") != std::string::npos);
  std::filesystem::remove_all(proc_root);
  ...
  Request first_request = LiveProcessRequest(first);
  Request invalid_second = LiveProcessRequest(second);
  invalid_second.expected_cgroup += "moved";
  assert(!TerminateMatchingProcesses({first_request, invalid_second}, "/proc",
                                     std::chrono::milliseconds(20), &error));
  assert(kill(first, 0) == 0);
  assert(kill(first, SIGKILL) == 0);
  assert(kill(second, SIGKILL) == 0);
  int status = 0;
  assert(waitpid(first, &status, 0) == first);
  assert(waitpid(second, &status, 0) == second);

  if (pidfd_supported) {
    const pid_t zombie_child = fork();
    assert(zombie_child >= 0);
    if (zombie_child == 0) {
      for (;;) {
        pause();
      }
    }
    const Request zombie_request = LiveProcessRequest(zombie_child);
    assert(TerminateMatchingProcesses(
        {zombie_request}, "/proc", std::chrono::seconds(2), &error));
    int zombie_status = 0;
    assert(waitpid(zombie_child, &zombie_status, 0) == zombie_child);
    assert(WIFSIGNALED(zombie_status));

    const pid_t pinned_child = fork();
    assert(pinned_child >= 0);
    if (pinned_child == 0) {
      for (;;) {
        pause();
      }
    }
    const Request pinned_request = LiveProcessRequest(pinned_child);
    const auto fake_proc_root =
        std::filesystem::temp_directory_path() /
        ("daemon-pinned-proc-loss-" +
         std::to_string(static_cast<unsigned long>(getpid())));
    const auto fake_process_dir =
        fake_proc_root / std::to_string(pinned_request.pid);
    std::filesystem::remove_all(fake_proc_root);
    std::filesystem::create_directories(fake_process_dir);
    std::filesystem::copy_file(
        "/proc/" + std::to_string(pinned_child) + "/stat",
        fake_process_dir / "stat");
    std::filesystem::copy_file(
        "/proc/" + std::to_string(pinned_child) + "/cgroup",
        fake_process_dir / "cgroup");
    std::vector<std::shared_ptr<PinnedProcess>> pinned;
    assert(PinMatchingProcesses({pinned_request}, fake_proc_root.string(),
                                &pinned, &error));
    assert(pinned.size() == 1);
    // Once pinned, even persistent loss of the /proc view cannot make shutdown
    // release ownership under this live process or redirect the signal to a
    // later process that reuses its numeric PID.
    std::filesystem::remove_all(fake_proc_root);
    std::thread pinned_reaper([pinned_child] {
      int child_status = 0;
      assert(waitpid(pinned_child, &child_status, 0) == pinned_child);
      assert(WIFSIGNALED(child_status));
    });
    assert(TerminatePinnedProcesses(pinned, std::chrono::seconds(2), &error));
    pinned_reaper.join();
    bool exited = false;
    assert(InspectPinnedProcess(*pinned.front(), &exited, &error));
    assert(exited);
  }
}
...
int main() {
  ...
  TestTerminateMatchingProcesses();
