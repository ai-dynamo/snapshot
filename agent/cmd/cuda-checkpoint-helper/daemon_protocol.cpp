...
bool IsAllowedStorageDirectory(const std::string &value,
  ...
  if (!root.is_absolute() || root.lexically_normal() != root ||
      !path.is_absolute() || path.lexically_normal() != path || path == root) {
    return false;
  }
...
bool ParseCgroupEntries(
  ...
  while (std::getline(lines, line)) {
    ...
    if (first_colon == std::string::npos ||
        second_colon == std::string::npos || second_colon + 1 >= line.size()) {
      return false;
    }
    const std::string hierarchy = line.substr(0, second_colon);
    if (hierarchy.empty() || entries->count(hierarchy) != 0) {
      return false;
    }
    std::vector<std::string> components;
    std::istringstream path(line.substr(second_colon + 1));
    std::string component;
    bool saw_named_component = false;
    while (std::getline(path, component, '/')) {
      if (component.empty()) {
        continue;
      }
      if (component == ".." && !saw_named_component) {
        continue;
      }
    ...
    entries->emplace(hierarchy, std::move(components));
  }
  return !entries->empty();
}

bool CgroupIdentitiesEquivalent(const std::string &expected,
                                const std::string &current) {
  if (expected == current) {
    return true;
  }
  std::map<std::string, std::vector<std::string>> expected_entries;
  std::map<std::string, std::vector<std::string>> current_entries;
  if (!ParseCgroupEntries(expected, &expected_entries) ||
      !ParseCgroupEntries(current, &current_entries) ||
      expected_entries.size() != current_entries.size()) {
    return false;
  }
  for (const auto &[hierarchy, expected_path] : expected_entries) {
    const auto current_entry = current_entries.find(hierarchy);
    if (current_entry == current_entries.end()) {
      return false;
    }
    const auto &current_path = current_entry->second;
    if (expected_path == current_path) {
      continue;
    }
    const auto &shorter = expected_path.size() < current_path.size()
                              ? expected_path
                              : current_path;
    const auto &longer = expected_path.size() < current_path.size()
                             ? current_path
                             : expected_path;
    // A cgroup namespace may hide ancestors, but the pod and container scopes
    // remain visible. Requiring both suffix components prevents a same-named
    // leaf in another pod from satisfying the identity pin.
    if (shorter.size() < 2 ||
        !std::equal(shorter.rbegin(), shorter.rend(), longer.rbegin())) {
      return false;
    }
  }
  return true;
...
bool ParseRequest(const unsigned char *data, size_t size, Request *request,
                  std::string *error, const std::string &storage_root) {
  if (data == nullptr || request == nullptr ||
      size < kLegacyRequestHeaderSize ||
      size > kMaxRequestSize) {
    *error = "invalid request size";
    return false;
  }
  const uint16_t version = ReadU16(data + 4);
  const size_t header_size = ReadU16(data + 6);
  const bool current_header =
      version == kVersion && header_size == kRequestHeaderSize;
  const bool legacy_header =
      version == kLegacyVersion && header_size == kLegacyRequestHeaderSize;
  if (ReadU32(data) != kMagic ||
      (!current_header && !legacy_header) || size < header_size) {
  ...
  if (current_header) {
    parsed.expected_job_file_device = ReadU64(data + 60);
    parsed.expected_job_file_inode = ReadU64(data + 68);
  }
  const char *payload =
      reinterpret_cast<const char *>(data + header_size);
  ...
  const unsigned char *batch_payload =
      data + header_size + device_map_size + storage_dir_size +
  ...
  if (action == Action::kRestoreBatch) {
    if (parsed.backend != Backend::kPosix || parsed.pid < 2 ||
        ...
        parsed.expected_job_file_device != 0 ||
        parsed.expected_job_file_inode != 0 ||
    ...
    while (offset < batch_size) {
      ...
      if (entry_size < kLegacyRequestHeaderSize ||
          entry_size > batch_size - offset) {
  ...
  if ((parsed.job_file.empty() &&
       (parsed.expected_job_file_device != 0 ||
        parsed.expected_job_file_inode != 0)) ||
      (parsed.expected_job_file_inode == 0 &&
       parsed.expected_job_file_device != 0)) {
    *error = "request has an invalid expected job-file identity";
    return false;
  }
  if (current_header && action == Action::kRestore &&
      !parsed.job_file.empty() && parsed.expected_job_file_inode == 0) {
    *error = "version 8 restore job file requires an expected identity";
    return false;
  }
      ...
      if (parsed.transfer_buffer_count != 0 ||
        ...
        *error = "regular backend request has custom-storage arguments";
        return false;
               ...
               parsed.transfer_chunk_bytes == 0 || parsed.storage_dir.empty() ||
               !IsAllowedStorageDirectory(parsed.storage_dir, storage_root) ||
               parsed.selected_devices.empty()) {
...
bool EncodeRequest(const Request &request, std::vector<unsigned char> *data,
  ...
  WriteU64(data, 60, request.expected_job_file_device);
  WriteU64(data, 68, request.expected_job_file_inode);
...
ProcessIdentityState InspectProcessIdentity(const Request &request,
  ...
  for (size_t index = 0; index < 20; ++index) {
    ...
    if (index == 0) {
      if (field.size() != 1) {
        *error = "invalid current process state";
        return ProcessIdentityState::kIndeterminate;
      }
      process_state = field.front();
    }
    if (index == 19) {
      char *end = nullptr;
      errno = 0;
  ...
  // A zombie can remain visible in procfs indefinitely when its namespace
  // parent does not reap it. It can no longer execute CUDA work, so waiting for
  // the PID directory to disappear would prevent safe context release during
  // PageBroker shutdown.
  if (process_state == 'Z') {
    *error = "current process is a zombie";
    return ProcessIdentityState::kExitedOrReused;
  }
  ...
  if (!CgroupIdentitiesEquivalent(request.expected_cgroup, cgroup)) {
...
PinnedProcess::~PinnedProcess() noexcept {
  if (pidfd_ >= 0)
    (void)close(pidfd_);
}
...
bool PinMatchingProcesses(
  ...
  if (targets == nullptr || error == nullptr) {
    return false;
  }
  targets->clear();
  for (const auto &request : requests) {
    std::string identity_error;
    if (InspectProcessIdentity(request, proc_root, &identity_error) ==
        ProcessIdentityState::kIndeterminate) {
      *error = "cannot safely pin target " + std::to_string(request.pid) +
               ": " + identity_error;
      return false;
    }
  }
  for (const auto &request : requests) {
    ...
    if (InspectProcessIdentity(request, proc_root, &identity_error) !=
      ...
      *error = "target " + std::to_string(request.pid) +
               " exited or changed identity before it could be pinned";
      targets->clear();
      return false;
    }
    const int pidfd = static_cast<int>(
        syscall(SYS_pidfd_open, static_cast<pid_t>(request.pid), 0));
    if (pidfd < 0) {
      const int open_error = errno;
      targets->clear();
      *error = "pidfd_open target " + std::to_string(request.pid) + ": " +
               std::strerror(open_error);
      return false;
    }
    const auto post_open_state =
        InspectProcessIdentity(request, proc_root, &identity_error);
    if (post_open_state != ProcessIdentityState::kMatches) {
      (void)close(pidfd);
      targets->clear();
      *error = post_open_state == ProcessIdentityState::kIndeterminate
                   ? "cannot safely pin target " +
                         std::to_string(request.pid) + ": " + identity_error
                   : "target " + std::to_string(request.pid) +
                         " changed identity while it was being pinned";
      return false;
    }
    PinnedProcess *pinned = nullptr;
    try {
      // Keep ownership local until the Request copy in PinnedProcess has
      // completed. If allocation or construction fails, the raw pidfd must not
      // escape this function.
      pinned = new PinnedProcess(request, pidfd);
    } catch (const std::bad_alloc &) {
      (void)close(pidfd);
      targets->clear();
      *error = "allocate pinned target " + std::to_string(request.pid);
      return false;
    }
    try {
      // shared_ptr deletes pinned if its control-block allocation fails.
      targets->push_back(std::shared_ptr<PinnedProcess>(pinned));
    } catch (const std::bad_alloc &) {
      targets->clear();
      *error = "retain pinned target " + std::to_string(request.pid);
      return false;
    }
  }
  error->clear();
...
bool InspectPinnedProcess(const PinnedProcess &target, bool *exited,
                          std::string *error) {
  if (exited == nullptr || error == nullptr || target.descriptor() < 0) {
    return false;
  }
  pollfd descriptor{.fd = target.descriptor(), .events = POLLIN, .revents = 0};
  int status;
  do {
    status = poll(&descriptor, 1, 0);
  } while (status < 0 && errno == EINTR);
  if (status < 0) {
    *error = "poll pinned target " +
             std::to_string(target.request().pid) + ": " +
             std::strerror(errno);
    return false;
  }
  if ((descriptor.revents & POLLNVAL) != 0) {
    *error = "pinned target descriptor is invalid for target " +
             std::to_string(target.request().pid);
    return false;
  }
  *exited = status > 0 &&
...
bool TerminatePinnedProcesses(
  ...
  for (const auto &target : targets) {
    if (target == nullptr || target->descriptor() < 0) {
      *error = "cannot signal an invalid pinned CUDA target";
      return false;
    }
    if (syscall(SYS_pidfd_send_signal, target->descriptor(), SIGKILL, nullptr,
                0) != 0 &&
        errno != ESRCH) {
      *error = "pidfd_send_signal target " +
               std::to_string(target->request().pid) + ": " +
               std::strerror(errno);
      return false;
    }
  ...
  for (;;) {
    ...
    for (const auto &target : targets) {
      bool exited = false;
      if (!InspectPinnedProcess(*target, &exited, error)) {
        return false;
      }
      all_exited = all_exited && exited;
    }
    if (all_exited) {
      error->clear();
      return true;
    ...
    if (std::chrono::steady_clock::now() >= deadline) {
      *error = "timed out confirming pinned CUDA targets terminated";
      return false;
    }
...
bool TerminateMatchingProcesses(const std::vector<Request> &requests,
  ...
  for (const auto &request : requests) {
    std::string identity_error;
    const auto state = InspectProcessIdentity(request, proc_root, &identity_error);
    if (state == ProcessIdentityState::kIndeterminate) {
      *error = "cannot safely terminate target " +
               std::to_string(request.pid) + ": " + identity_error;
      return false;
    }
    if (state == ProcessIdentityState::kMatches)
  ...
  std::vector<std::shared_ptr<PinnedProcess>> targets;
  if (!PinMatchingProcesses(live, proc_root, &targets, error))
    return false;
  return TerminatePinnedProcesses(targets, timeout, error);
}
...
OperationHealth::OperationHealth(std::chrono::seconds max_operation_duration,
    ...
    : max_operation_duration_(max_operation_duration),
      incarnation_(std::move(incarnation)) {}
...
bool GenerateIncarnation(std::string *value, std::string *error) {
  ...
  while (offset < bytes.size()) {
    ...
    *error = std::string("generate daemon incarnation: ") +
             (count < 0 ? std::strerror(errno) : "short random read");
    return false;
  }
