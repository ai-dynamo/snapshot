...
bool ReadManifestContentsFile(int manifest_fd, std::string *contents,
                              std::string *error) {
  struct stat manifest_stat {};
  if (manifest_fd < 0 || fstat(manifest_fd, &manifest_stat) != 0 ||
      !S_ISREG(manifest_stat.st_mode) || manifest_stat.st_size < 0 ||
      static_cast<uintmax_t>(manifest_stat.st_size) > kMaximumManifestSize) {
    *error = "helper manifest is not a bounded regular file";
    return false;
  ...
  size_t offset = 0;
  while (offset < contents->size()) {
    const ssize_t bytes = pread(manifest_fd, contents->data() + offset,
  ...
  ssize_t bytes = -1;
  do {
    bytes = pread(manifest_fd, &trailing, sizeof(trailing),
...
bool ParseManifestContents(const std::string &contents,
...
bool ReadManifest(const std::filesystem::path &directory,
  ...
  return ParseManifestContents(contents, extents, error);
}

bool ReadManifestFile(int manifest_fd, std::vector<ManifestExtent> *extents,
                      std::string *error) {
  if (extents == nullptr || error == nullptr)
    return false;
  std::string contents;
  if (!ReadManifestContentsFile(manifest_fd, &contents, error))
    return false;
  return ParseManifestContents(contents, extents, error);
}
...
bool ValidateExtentFilesAt(int directory_fd,
                           const std::vector<ManifestExtent> &extents,
                           std::string *error) {
  if (directory_fd < 0 || error == nullptr) {
    return false;
  }
  std::vector<ManifestExtent> normalized;
  if (!NormalizeManifest(extents, &normalized, error)) {
  ...
  for (const auto &extent : normalized) {
    const int fd = openat(directory_fd, extent.filename.c_str(),
    ...
    struct stat extent_stat{};
    const bool valid = fstat(fd, &extent_stat) == 0 &&
                       S_ISREG(extent_stat.st_mode) &&
                       extent_stat.st_size >= 0 &&
                       static_cast<size_t>(extent_stat.st_size) == extent.size;
