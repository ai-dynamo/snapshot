...
This directory contains the transfer-neutral CUDA checkpoint operation core. It defines the configuration, cancellation, transfer-backend, and durable storage-manifest contracts consumed behind the existing PageBroker boundary. It does not add a Snapshot-local daemon or coordinator or select PageBroker's production data plane.
...
- extents are identified by source GPU UUID, exact size, and deterministic filename;
...
The helper manifest uses version 4 and contains no content checksums. Earlier manifest versions are rejected. File-size and layout validation do not detect same-size payload corruption; checkpoint storage is trusted.
...
`make test` always runs the Go suite. When a C++20 compiler is available, it also runs the standalone manifest, transfer-configuration, and cancellation tests. Missing C++ prerequisites are fatal in CI and optional for local Go-only development.
