...
bool TestValidateExtentFilesAtPinsDirectoryAndRejectsCorruption() {
  char path[] = "/tmp/cuda-storage-extent-at-test-XXXXXX";
  const char *root = mkdtemp(path);
  if (!Check(root != nullptr, "mkdtemp failed")) {
    return false;
  }
  const std::filesystem::path directory =
      std::filesystem::path(root) / "source";
  const std::filesystem::path held = std::filesystem::path(root) / "held";
  const std::filesystem::path replacement =
      std::filesystem::path(root) / "replacement";
  std::filesystem::create_directories(directory);
  std::filesystem::create_directories(replacement);
  const std::string filename = storage::DeviceFilename(0);
  {
    std::ofstream(directory / filename, std::ios::binary) << "good";
    std::ofstream(replacement / filename, std::ios::binary) << "evil";
  }
  const int directory_fd =
      open(directory.c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC);
  if (!Check(directory_fd >= 0, "open extent directory failed")) {
    return false;
  }
  std::filesystem::rename(directory, held);
  std::filesystem::create_directory_symlink(replacement, directory);
  const std::vector<storage::ManifestExtent> extents{
      {kSourceA, 4, filename, ""},
  };
  std::string error;
  const bool pinned = storage::ValidateExtentFilesAt(
      directory_fd, extents, &error);
  const bool corrupted = truncate((held / filename).c_str(), 3) == 0;
  const bool rejected_corruption =
      corrupted && !storage::ValidateExtentFilesAt(directory_fd, extents,
                                                    &error);
  const bool closed = close(directory_fd) == 0;
  std::error_code ignored;
  std::filesystem::remove_all(root, ignored);
  return Check(pinned, "descriptor validation followed a replaced path") &&
         Check(corrupted, "failed to corrupt pinned extent fixture") &&
         Check(rejected_corruption,
               "descriptor validation accepted corrupted extent size") &&
         Check(closed, "close extent directory failed");
}
...
int main() {
  if (!TestGPUUUIDParsing() || !TestEqualSizeNonOrderPreservingMap() ||
      ...
      !TestValidateExtentFiles() ||
      !TestValidateExtentFilesAtPinsDirectoryAndRejectsCorruption() ||
      !TestRemoveManifest() ||
