...
int main(int argc, char **argv) {
  ...
  if (status == CUDA_ERROR_INVALID_VALUE || status == CUDA_ERROR_NOT_INITIALIZED) {
    ...
    if (existence == daemon_protocol::ProcessExistenceState::kExists) {
      // The output pointer is valid and the candidate PID still exists.
      // Driver revisions use INVALID_VALUE or NOT_INITIALIZED for a live
      // process without CUDA checkpoint state. Keep that negative result
      // distinct from helper, driver, and raced-PID failures so the agent can
