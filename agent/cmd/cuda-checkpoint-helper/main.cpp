...
DoCustomStorage(int pid, bool checkpoint, const std::string &device_map,
  ...
  extent_digests.reserve(worker_metrics.size());
  for (const auto &metrics : worker_metrics) {
...
int RunDaemon(const std::string &socket_path, uint64_t max_operation_seconds) {
  ...
  const OperationCompleteFn operation_complete = ResolveOperationComplete(
  ...
  const OperationCompleteFn custom_storage_operation_complete =
  ...
  daemon_protocol::OperationHealth operation_health{
      std::chrono::seconds(max_operation_seconds), incarnation};
  ...
  bool daemon_fatal = false;
  {
    ...
    try {
      std::fprintf(
          ...
          custom_storage_available ? "true" : "false", incarnation.c_str());
      std::fflush(stdout);
  ...
  while (termination_status != CUDA_SUCCESS) {
    ...
    termination_status =
        persistent_contexts.TerminateAll("/host/proc", &termination_error);
