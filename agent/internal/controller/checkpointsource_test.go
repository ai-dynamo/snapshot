...
const recordedImageDigest = "sha256:9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c9f2c"
...
func recordedSource() *snapshotv1alpha1.CheckpointSource {
	return &snapshotv1alpha1.CheckpointSource{
		...
		Pod: &snapshotv1alpha1.CheckpointSourcePod{
			Image:       "nvcr.io/nvidia/ai-dynamo/vllm-runtime:0.6.1",
			ImageDigest: recordedImageDigest,
			Memory:      "64Gi",
			CPU:         "16",
		},
	}
...
// Each case compares the whole projection, so a value the agent could not read
	...
	for name, tc := range map[string]struct {
		...
		"an image ID the runtime reported as a reference": {
			manifest: manifestWith(func(m *types.CheckpointManifest) {
				m.K8s.ImageID = "nvcr.io/nvidia/ai-dynamo/vllm-runtime@" + recordedImageDigest
			}),
			want: recordedSource(),
		},
		"a CPU capture": {
...
// committedArtifact writes a manifest carrying a source where the agent will look
...
// The artifact is already committed by the time this runs, so losing the source
...
// source has to survive that: it comes from the artifact, not from memory.
