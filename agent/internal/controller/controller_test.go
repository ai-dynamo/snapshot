...
func liveRestoredContainerID(t *testing.T, w *NodeController, pod *corev1.Pod, destination string) string {
	...
	return live.Annotations[podcontract.RestoredContainerIDAnnotationKey(destination)]
}
...
func TestTerminalRestorePodWithNoNewIncarnationStillReportsAlreadyCompleted(t *testing.T) {
	pod := restorePod(map[string]string{
		podcontract.RestoreFromAnnotation:                    "snapshot-a",
		podcontract.RestoredContainerIDAnnotationKey("main"): testContainerID,
	})
	pod.Finalizers = []string{restorePodFinalizer}
	pod.Status.Conditions = append(pod.Status.Conditions, corev1.PodCondition{
...
func TestRestoreSkipsDestinationWhoseIncarnationIsAlreadyRestored(t *testing.T) {
	pod := multiRestorePod()
	pod.Finalizers = []string{restorePodFinalizer}
	pod.Annotations[podcontract.RestoredContainerIDAnnotationKey("engine-0")] = "engine-0-id"
	pod.Annotations[podcontract.RestoredContainerIDAnnotationKey("engine-1")] = "engine-1-id"
	pod.Status.Conditions = append(pod.Status.Conditions, corev1.PodCondition{
		Type: corev1.PodConditionType(podcontract.RestoredCondition), Status: corev1.ConditionTrue, Reason: podcontract.RestoreReasonSucceeded,
	})
...
func TestSuccessfulRestoreRecordsContainerIDAnnotation(t *testing.T) {
	...
	assert.Equal(t, testContainerID, pod.Annotations[podcontract.RestoredContainerIDAnnotationKey("main")], "the in-memory Pod must see the record for the rest of the pass")
}
...
func TestRestoreDestinationFailsWhenIncarnationRecordCannotBePersisted(t *testing.T) {
	...
	w.clientset.(*fake.Clientset).PrependReactor("patch", "pods", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		...
		if !strings.Contains(string(patch.GetPatch()), podcontract.RestoredContainerIDAnnotationPrefix) {
