...
func TestRestoreConditionTerminal(t *testing.T) {
	...
	pod.Status.Conditions[len(pod.Status.Conditions)-1].Reason = restoreCleanupFailedReason
	assert.True(t, restoreConditionTerminal(pod))
...
func TestRunRestoreCleanupFailureUsesDedicatedConditionReason(t *testing.T) {
	pod := restorePod(map[string]string{snapshotv1alpha1.RestoreFromAnnotation: "snapshot-a"})
	w := makeTestController(t, pod)
	...
	w.restoreFn = func(_ context.Context, _ snapshotruntime.Runtime, _ logr.Logger, got executor.RestoreRequest, _ executor.Mounters) (int, error) {
		request = got
		return 0, &executor.RestoreCleanupError{Action: "unmount checkpoint artifact", Err: errors.New("unmount failed")}
	}

	err := w.runRestore(context.Background(), pod, artifact, "ctr-abc", "inference/restore-worker/main/ctr-abc", time.Time{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "placeholder PID could not be resolved")
	assert.Equal(t, "content-uid", request.ContentUID)
	assert.Equal(t, w.config.Storage.BasePath, request.BasePath)
	assert.True(t, sawEventReason(w.clientset.(*fake.Clientset), restoreCleanupFailedReason))

	var terminalPatch clientgotesting.PatchAction
	for _, action := range w.clientset.(*fake.Clientset).Actions() {
		patch, ok := action.(clientgotesting.PatchAction)
		if ok && patch.GetSubresource() == "status" && strings.Contains(string(patch.GetPatch()), restoreCleanupFailedReason) {
			terminalPatch = patch
		}
	}
	require.NotNil(t, terminalPatch)
	assert.Contains(t, string(terminalPatch.GetPatch()), `"status":"False"`)
}
