...
func Restore(ctx context.Context, rt snapshotruntime.Runtime, log logr.Logger, req RestoreRequest, mounts RestoreMounter) (placeholderPID int, retErr error) {
	...
	result, err := execNSRestore(
		ctx, log, req, snap, bundleMount, containerCheckpointPath,
	)
	if err != nil {
		_, _ = awaitPrefetch(true)
	...
	if err != nil {
		if ctx.Err() != nil {
			return 0, fmt.Errorf(
				"CUDA CustomStorage artifact prefetch interrupted after CRIU restore: %w",
				err,
			)
		}
		...
		log.Error(
			err,
			"CUDA CustomStorage artifact prefetch failed; continuing with authoritative restore",
		)
