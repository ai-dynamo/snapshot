...
func Checkpoint(ctx context.Context, rt snapshotruntime.Runtime, log logr.Logger, req CheckpointRequest, cfg *types.AgentConfig) (retErr error) {
	...
	if brokered {
		...
		if err != nil {
			return checkpointPreMutationError(
				fmt.Errorf("prepare PageBroker checkpoint: %w", err),
			)
		}
	} else {
		...
		if err != nil {
			return checkpointPreMutationError(
				fmt.Errorf("resolve checkpoint staging root: %w", err),
			)
		}
		if err := os.MkdirAll(tmpRoot, 0700); err != nil {
			return checkpointPreMutationError(
				fmt.Errorf("failed to create checkpoint staging root: %w", err),
			)
		}
		if err := os.MkdirAll(filepath.Dir(finalDir), 0700); err != nil {
			return checkpointPreMutationError(
				fmt.Errorf("failed to create checkpoint container root: %w", err),
			)
		}
		tmpDir = filepath.Join(tmpRoot, transactionID)
		if err := os.Mkdir(tmpDir, 0700); err != nil {
			return checkpointPreMutationError(
				fmt.Errorf("failed to create checkpoint staging directory: %w", err),
			)
		}
