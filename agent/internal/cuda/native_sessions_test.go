...
func TestNativeSessionsGlobalTransferBarrier(t *testing.T) {
	for _, failure := range []bool{false, true} {
		t.Run(fmt.Sprint("transfer-failure=", failure), func(t *testing.T) {
			...
			err := RunNativeSessions(context.Background(), sessions, []int{10, 20}, false, true, logr.Discard(), []int{110, 120})
			mutex.Lock()
			defer mutex.Unlock()
			if (err != nil) != failure {
...
func TestNativeSessionsRejectMissingCapability(t *testing.T) {
	if err := RunNativeSessions(context.Background(), map[string]*os.File{}, []int{1}, false, true, logr.Discard()); err == nil {
