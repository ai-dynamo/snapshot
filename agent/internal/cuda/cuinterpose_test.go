...
// Keep endpoint discovery and artifact names aligned without a second
// wire-protocol implementation. The packaged verifier checks the actual CLI.
func TestGoConstantsMatchTheShimSources(t *testing.T) {
	root := filepath.Join("..", "..", "cmd", "cuinterpose", "cpp")
	core, err := os.ReadFile(filepath.Join(root, "process.cpp"))
	if err != nil {
		t.Fatal(err)
	}
	coordinator, err := os.ReadFile(filepath.Join(root, "coordinator.cpp"))
	if err != nil {
		t.Fatal(err)
	}
	for _, value := range []string{cuinterposeSocketPrefix, podcontract.SnapshotControlMountPath, podcontract.SnapshotControlDirEnv} {
		if !strings.Contains(string(core), value) {
			t.Errorf("core lacks endpoint contract %q", value)
		}
	}
	if !strings.Contains(string(coordinator), `"/`+CuinterposeStateFile+`"`) {
		t.Error("coordinator state filename differs from Go")
	}
}
