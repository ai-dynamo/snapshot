...
func BindNativeSessions(ctx context.Context, broker pagebroker.Client, transaction string, containerPID int, pids []int, devices []string, deviceMap string, save bool) (AllocationSessions, error) {
	direction := pagebroker.BindAllocationSession_LOAD
	if save {
		direction = pagebroker.BindAllocationSession_SAVE
	}
	sessions := make(AllocationSessions, len(pids))
...
func RunNativeSessions(ctx context.Context, sessions AllocationSessions, pids []int, save, pipeline bool, log logr.Logger, observed ...[]int) error {
	start := time.Now()
