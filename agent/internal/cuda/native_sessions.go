...
func BindNativeSessions(ctx context.Context, broker pagebroker.Client, transaction string, containerPID int, pids []int, devices []string, deviceMap string, save bool) (NativeSessions, error) {
	direction := pagebroker.BindNativeSession_LOAD
	if save {
		direction = pagebroker.BindNativeSession_SAVE
	}
...
func RunNativeSessions(ctx context.Context, sessions NativeSessions, pids []int, save bool, log logr.Logger, observed ...[]int) error {
	start := time.Now()
	...
	log.Info("Native PageBroker batch complete", "save", save, "duration", time.Since(start))
	return nil
}
