...
Deployment runs a `crd-installer` init container before the manager starts. The
operator image carries the same manifests at `/etc/snapshot/crds` — both they
and `crds/` are generated by `make generate` from the `api/` types the operator
compiles against — and the init container server-side applies them, so every
rollout converges the cluster on definitions the running binary agrees with.
Nothing to update means nothing is written:
