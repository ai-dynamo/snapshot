...
func TestCUDAToolsInstallIncludesBothShimLibraries(t *testing.T) {
	init := expectedCUDAToolsInitContainer(testDelivery())
	want := []string{
		"--", cudaToolsImageCUDACheckpoint, cudaToolsImageLibrary, cudaToolsImageCoreLibrary,
		cudaToolsInstallMountPath + "/",
	}
	if !reflect.DeepEqual(init.Args, want) {
		t.Fatalf("install args = %v, want all three tools: %v", init.Args, want)
	}
	for _, missing := range []string{cudaToolsImageLibrary, cudaToolsImageCoreLibrary} {
		t.Run(missing, func(t *testing.T) {
			partial := init.DeepCopy()
			partial.Args = nil
			...
			if initContainerMatches(partial, &init) {
				t.Fatal("an adopted install missing a shim library must be rejected")
			}
		})
	}
}
