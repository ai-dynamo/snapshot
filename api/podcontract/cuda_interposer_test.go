...
func TestShapeCUDAInterposerCapture(t *testing.T) {
	template := &corev1.PodTemplateSpec{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{
				...
				{Name: "worker-b", Command: []string{"/usr/bin/worker"}},
				{Name: "helper"},
			},
		},
	}
	...
	assertCUDAInterposerLaunchJob(
		t,
		findContainer(&template.Spec, "worker-a"),
		[]string{"python3", "-m", "worker", "--rank", "0"},
	)
	assertCUDAInterposerLaunchJob(
		t,
		findContainer(&template.Spec, "worker-b"),
		[]string{"/usr/bin/worker"},
	)
...
func TestShapeCUDAInterposerCaptureRejectsInvalidContract(t *testing.T) {
	tests := map[string]*corev1.PodTemplateSpec{
		"invalid annotation": {
			...
			Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "worker", Command: []string{"worker"}}}},
		},
...
func assertCUDAInterposerLaunchJob(t *testing.T, container *corev1.Container, original []string) {
	t.Helper()
	if !reflect.DeepEqual(container.Command, []string{CUDACheckpointBinaryPath}) {
