...
// Delivery never rewrites the application command. A Pod opts into the
// otherwise inert cuinterpose library separately (see cuinterpose.go).
const (
	...
	// CUDACheckpointPath is the delivered native checkpoint tool.
	...
	// CuinterposeCoreLibraryPath is loaded lazily beside the frontend.
	CuinterposeCoreLibraryPath = CUDAToolsMountPath + "/libcuinterpose_core.so"
	...
	cudaToolsImageCoreLibrary    = "/usr/local/lib/snapshot/libcuinterpose_core.so"
...
// read-only in every target, and leaves the workload command unchanged.
// Reapplying it is a no-op. On error the template is left unchanged.
func ShapeCUDATools(
	...
	if podTemplate == nil {
		return fmt.Errorf("CUDA tools require a pod template")
	}
	if len(targetContainers) == 0 {
		return fmt.Errorf("CUDA tools require at least one target container")
	}
...
func expectedCUDAToolsInitContainer(delivery CUDAToolsDelivery) corev1.Container {
	...
	return corev1.Container{
		...
		Args: []string{
			...
			cudaToolsImageCoreLibrary,
			cudaToolsInstallMountPath + "/",
		},
		// The agent image runs as root and only copies three files into a
...
func initContainerMatches(actual, expected *corev1.Container) bool {
	...
	return slices.EqualFunc(actual.VolumeMounts, expected.VolumeMounts, func(a, b corev1.VolumeMount) bool {
