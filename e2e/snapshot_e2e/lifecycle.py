...
def _dump_agent_diagnostics(config: k8s.E2EConfig, run: TestRun, source_node: str) -> None:
    ...
    _dump_section(
        "pagebroker logs",
        lambda: _dump_pagebroker_logs(config, agent, source_node),
    )
...
def _dump_pagebroker_logs(config: k8s.E2EConfig, agent: str, source_node: str) -> None:
    print(f"--- pagebroker {agent} on {source_node} (tail 200) ---")
    print(
        k8s.pod_logs(
            config.namespace,
            agent,
            tail_lines=200,
            container=PAGEBROKER_CONTAINER,
        )
    )
