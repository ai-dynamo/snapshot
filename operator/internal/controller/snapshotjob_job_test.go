...
func TestBuildSourceJob(t *testing.T) {
	...
	t.Run("command/args pass through unchanged", func(t *testing.T) {
		sj := minimalSnapshotJob()
...
func buildSourceJob(sj *snapshotv1alpha1.SnapshotJob) (*batchv1.Job, error) {
	return buildShapedSourceJob(sj, testCUDAToolsDelivery())
}

func TestBuildShapedSourceJobPreservesMultiGPUCommands(t *testing.T) {
	sj := multiGPUSnapshotJob()

	job, err := buildShapedSourceJob(sj, testCUDAToolsDelivery())
	require.NoError(t, err)
	...
	t.Run("every shaped SnapshotJob needs a configured agent image", func(t *testing.T) {
		_, err := buildShapedSourceJob(minimalSnapshotJob(), podcontract.CUDAToolsDelivery{})
		require.Error(t, err)
	})
	...
	t.Run("DRA claims do not change the workload command", func(t *testing.T) {
		sj := minimalSnapshotJob()
		worker := &sj.Spec.PodTemplate.Spec.Containers[0]
		...
		job, err := buildShapedSourceJob(sj, testCUDAToolsDelivery())
		require.NoError(t, err)
		assert.Equal(t, []string{"worker"}, job.Spec.Template.Spec.Containers[0].Command)
	})
}
...
func TestBuildShapedSourceJobShapesCuinterpose(t *testing.T) {
	...
	t.Run("a SnapshotJob without the annotation does not preload the shim", func(t *testing.T) {
		job, err := buildShapedSourceJob(multiGPUSnapshotJob(), testCUDAToolsDelivery())
		require.NoError(t, err)
		require.NoError(t, podcontract.VerifyCUDATools(&job.Spec.Template.Spec, []string{"worker"}))
