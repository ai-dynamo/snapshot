...
func RestoreStatusAnnotationKeysFor(containerName string) (snapshotv1alpha1.RestoreStatusAnnotationKeys, error) {
	keys := snapshotv1alpha1.RestoreStatusAnnotationKeys{
	...
	for _, annotationKey := range []string{keys.Status, keys.ContainerID} {
		if errs := validation.IsQualifiedName(annotationKey); len(errs) > 0 {
			return snapshotv1alpha1.RestoreStatusAnnotationKeys{}, fmt.Errorf("container name %q cannot be used in restore status annotation key %q: %s", containerName, annotationKey, strings.Join(errs, "; "))
		}
	}
	return keys, nil
}
...
func ApplyCheckpointStorageMetadata(annotations map[string]string, storage snapshotv1alpha1.Storage) {
	if annotations == nil {
		return
	}
