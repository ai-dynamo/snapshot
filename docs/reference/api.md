...
| `source.devices.nvidia.instances[].migProfile` | string | MIG slice shape the capture ran on, such as `1g.10gb`. Absent when the capture had a whole GPU. `productName` does not imply it: `nvidia-smi` reports a slice under its parent card's model name. |
