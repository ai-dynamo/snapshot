...
For opted-in workloads, Snapshot copies `libcuinterpose.so` and `libcuinterpose_core.so` from the agent image into an `emptyDir` mounted read-only at `/tmp/snapshot-cuda` and prepends the frontend to `LD_PRELOAD`, preserving existing entries. Workload commands are unchanged. Unannotated workloads receive no shim init container or mount. Native CUDA checkpoint and restore use `cuda-checkpoint-helper`, which calls the CUDA driver API directly.

Existing CUDA jobfiles are still staged, refreshed after native checkpoint, and restored through the native helper. A cuinterpose workload may run with or without one: only the missing-jobfile requirement is waived. Unannotated multi-GPU workloads retain the native jobfile requirement. SnapshotJob leaves command wrapping disabled; the optional `snapshotctl --cuda-checkpoint-wrap` path remains available.
