...
This selects all supported creator-owned, device-pinned `cuMemCreate` allocations, whether or not the application exported them. The legacy-memory adapter also contributes tracked VMM records for supported synchronous allocations and memory IPC. Other allocator families and CUDA process state remain native-owned. Conditional `--launch-job` wrapping is unchanged in this mode; native CustomStorage is not used.

A separate `nvidia.com/cuinterpose-allocation-storage: custom-storage` mode keeps shared contents in host carriers and sends native private CUDA contents through PageBroker's CustomStorage worker. It requires the same interposition and PageBroker opt-ins, but explicitly omits `--launch-job`: the memory-IPC adapter removes supported legacy-memory dependencies before native capture. The agent validates complete shim coverage and rejects jobfile state in this mode. Native restore completes before shim sharing is reconstructed and before application release. See [native CustomStorage transfers](../../agent/pagebroker/custom_storage.md) for the transaction, namespace, GPU-remapping, and fail-closed lifecycle. This does not extend support to IPC events, memory-pool IPC, or foreign native IPC handles.
